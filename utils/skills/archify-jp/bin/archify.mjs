...
function usage() {
  ...
  archify guide [シナリオまたは質問] [--json] [--lang ja|en|zh]
  archify brands [名前、別名、ドメイン、またはカテゴリー] [--json]
...
function rendererPath(type) {
  if (!TYPES.has(type)) {
    fail(`不明な図の種類です: "${type}"。次のいずれかを指定してください: ${[...TYPES].join(', ')}`);
  }
...
function extractQualityArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--quality') {
      ...
      if (!quality || quality.startsWith('--')) fail('--qualityにはstandardまたはshowcaseの指定が必要です。');
      index += 1;
      continue;
    }
    if (arg.startsWith('--quality=')) {
      quality = arg.slice('--quality='.length);
      if (!quality) fail('--qualityにはstandardまたはshowcaseの指定が必要です。');
      continue;
    }
    rest.push(arg);
  }
  if (quality !== undefined && !['standard', 'showcase'].includes(quality)) {
    fail(`不明な品質プロファイルです: "${quality}"。standardまたはshowcaseを指定してください。`);
  }
  return { rest, quality };
}
...
function extractRepoRootArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--repo-root') {
      ...
      if (!repoRoot || repoRoot.startsWith('--')) fail('--repo-rootにはリポジトリのパスが必要です。');
      index += 1;
      continue;
    }
    if (arg.startsWith('--repo-root=')) {
      repoRoot = arg.slice('--repo-root='.length);
      if (!repoRoot) fail('--repo-rootにはリポジトリのパスが必要です。');
      continue;
    }
    rest.push(arg);
  }
...
function inputDiagnostic(error, inputPath) {
  ...
  return diagnostic({
    ...
    message: isSyntax
      ? `入力JSONを解析できませんでした: ${error.message}`
      : `入力を読み取れませんでした: ${error.message}`,
    ...
    supportedFixes: [isSyntax
      ? 'JSON構文を修正してから、もう一度検証を実行してください'
      : '読み取り可能なJSON入力ファイルを1つ指定してください'],
  });
}
...
function rendererFailure(result) {
  if (result.error) {
    return {
      error: 'Rendererプロセスを開始できませんでした。',
      diagnostics: [diagnostic({
        code: 'internal/renderer-process',
        message: 'Rendererプロセスを開始できませんでした。',
  ...
  return {
    error: 'Rendererは構造化された診断を出力する前に失敗しました。',
    diagnostics: [diagnostic({
      code: 'internal/unclassified',
      message: 'Rendererは構造化された診断を出力する前に失敗しました。',
...
const CHECK_FIXES = {
  single_svg: ['追加のSVGルートを削除し、成果物に図のSVGが1つだけ含まれるようにしてください'],
  finite_svg: ['再描画する前に有限でない座標を置き換えてください'],
  orthogonal_arrows: ['Rendererが対応する直交ルーティング制御を使用してください'],
  legend_clearance: ['関係線が凡例に入らないように、経路を移動するかviewBoxを拡大してください'],
};

const COMPOSITION_FIXES = {
  'composition/proper-crossing': ['無関係な関係線が別々の経路を通るように、route/viaまたはchannel座標を調整してください'],
  'composition/ambiguous-corridor': ['無関係な関係線が視覚的に合流しないように、route/viaまたはchannel座標を調整してください'],
  'composition/container-border-run': ['明確な開口部を通り、枠と直交するように経路を設定してください'],
  'composition/label-route-clearance': ['labelAt、labelDx、labelDy、labelSegment、メッセージのy座標、またはもう一方の関係線の経路を調整してください'],
  'composition/desktop-readability': ['1440pxのデスクトップ表示でノード内の文脈が6px以上になるよう、viewBoxの幅を縮める、ノード文言を短くする、対象ノードを広げる、または図を分割してください'],
  'composition/micro-segment': ['表示される各線分が8px以上になるように、route/channel/viaの点を移動してください'],
  'composition/short-interior-segment': ['内側の各折れ曲がりに16px以上の長さが確保されるように、route/channel/viaの点を移動してください'],
};
...
function checkerDiagnostics(checker) {
  ...
  for (const issue of checker?.composition?.issues || []) {
    ...
    diagnostics.push(diagnostic({
      code,
      severity,
      message: `最終成果物が${code}に合格しませんでした。`,
  ...
  for (const check of checker?.checks || []) {
    ...
    diagnostics.push(diagnostic({
      ...
      message: (check.details || []).find(Boolean) || `最終成果物が${check.name}に合格しませんでした。`,
  ...
  return diagnostics.length ? diagnostics : [diagnostic({
    ...
    message: '分類済みの診断がない状態で最終成果物のチェックに失敗しました。',
...
function formatDiagnostics(error, diagnostics = []) {
  ...
  return [
    ...
    ...diagnostics.map((entry) => {
      const fix = entry.supportedFixes?.length ? ` 修正方法: ${entry.supportedFixes.join('; ')}。` : '';
...
function extractCompareOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--receipt') {
      ...
      if (!receipt || receipt.startsWith('--')) fail('--receiptにはJSON出力パスが必要です。');
      index += 1;
      continue;
    }
    if (arg.startsWith('--receipt=')) {
      receipt = arg.slice('--receipt='.length);
      if (!receipt) fail('--receiptにはJSON出力パスが必要です。');
      continue;
    }
    if (arg.startsWith('--')) fail(`不明なcompareオプションです: "${arg}"。`);
    positional.push(arg);
  }
...
function commitComparePair({ htmlCandidate, receiptCandidate, outputPath, receiptPath, stagingDirectory }) {
  const targets = [
    { label: 'HTML成果物', target: outputPath, candidate: htmlCandidate, backup: path.join(stagingDirectory, '.previous-output') },
    { label: 'レシート', target: receiptPath, candidate: receiptCandidate, backup: path.join(stagingDirectory, '.previous-receipt') },
  ];
  ...
  for (const item of targets) {
    ...
    if (!existing.isFile()) {
      throw compareCommitError(
        `Architecture Deltaをコミットできませんでした: 既存の${item.label}の出力先が通常ファイルではありません。`,
        'delta/commit-target',
        {
          target: path.basename(item.target),
          targetType: existing.isDirectory() ? 'directory' : 'non-file',
          supportedFixes: [`${item.label}には通常ファイルのパスを指定してください`],
        },
      );
    }
  }
  ...
  try {
    ...
    for (const item of [...committed].reverse()) {
      try {
        ...
        rollbackErrors.push(`${item.label}: 削除に失敗しました（${error.message}）`);
      }
    }
    for (const item of [...backedUp].reverse()) {
      try {
        ...
        rollbackErrors.push(`${item.label}: 復元に失敗しました（${error.message}）`);
      }
    }
    throw compareCommitError(
      rollbackErrors.length
        ? 'Architecture Deltaの一括コミットに失敗し、以前のファイルを完全には復元できませんでした。'
        : 'Architecture Deltaの一括コミットに失敗しました。以前のファイルは復元されました。',
      rollbackErrors.length ? 'delta/commit-rollback-failed' : 'delta/commit-failed',
      {
        reason: cause.message,
        ...(rollbackErrors.length ? { rollbackErrors } : {}),
        supportedFixes: ['両方の出力パスが書き込み可能な通常ファイルであることを確認してから再試行してください'],
      },
    );
  }
}
...
function renderValidatedArchitecture(inputPath, outputPath, quality, repoRoot) {
  ...
  if (check.status !== 0) {
    const error = new Error('検証済みスナップショットが最終成果物のチェックに合格しませんでした。');
...
async function commandCompare(args) {
  ...
  try {
    ...
    reportCompareFailure({ json: options.json, stage: 'prepare', error: 'Architecture比較ランタイムを利用できません。', code: 'delta/runtime-missing', details: { reason: error.message, supportedFixes: ['完全なArchifyスキルパッケージをインストールしてください'] } });
    return;
  }
  const {
  ...
  try {
    ...
    reportCompareFailure({
      ...
      details: {
        ...
        supportedFixes: outputDiagnostic?.supportedFixes || ['安全な出力パスを指定して再試行してください'],
      },
    });
    return;
  }
  ...
  try {
    ...
    reportCompareFailure({
      ...
      details: {
        ...(outputDiagnostic?.subject || {}),
        ...(outputDiagnostic?.evidence || {}),
        supportedFixes: outputDiagnostic?.supportedFixes || ['安全なレシートパスを指定して再試行してください'],
      },
    });
    return;
  }
  ...
  try {
    ...
    reportCompareFailure({ json: options.json, stage: 'input', error: `base入力を読み取れませんでした: ${error.message}`, code: 'delta/base-input', details: { side: 'base', reason: error.message } });
    return;
  }
  try {
    ...
    reportCompareFailure({ json: options.json, stage: 'input', error: `head入力を読み取れませんでした: ${error.message}`, code: 'delta/head-input', details: { side: 'head', reason: error.message } });
    return;
  }

  const outputDirectory = path.dirname(outputPath);
  if (path.dirname(receiptPath) !== outputDirectory) {
    reportCompareFailure({ json: options.json, stage: 'prepare', error: 'compareレシートはHTML成果物と同じディレクトリに書き込む必要があります。', code: 'delta/receipt-directory', details: { supportedFixes: ['output.htmlと同じディレクトリの--receiptパスを指定してください'] } });
    return;
  }
  try {
    fs.mkdirSync(outputDirectory, { recursive: true });
  } catch (error) {
    reportCompareFailure({ json: options.json, stage: 'prepare', error: `compare出力ディレクトリを作成できませんでした: ${error.message}`, code: 'delta/output-directory', details: { reason: error.message } });
    return;
  }
  ...
  try {
    ...
    reportCompareFailure({ json: options.json, stage: 'prepare', error: `compare候補を作成できませんでした: ${error.message}`, code: 'delta/candidate-directory', details: { reason: error.message } });
    return;
  }
  ...
  try {
    ...
    try {
      ...
      reportCompareFailure({
        ...
        error: `baseスナップショットの検証に失敗しました: ${error.message}`,
    ...
    try {
      ...
      reportCompareFailure({
        json: options.json,
        stage: error.compareStage || 'validate',
        error: `headスナップショットの検証に失敗しました: ${error.message}`,
    ...
    try {
      ...
      reportCompareFailure({
        ...
        details: {
          ...(outputDiagnostic?.subject || {}),
          ...(outputDiagnostic?.evidence || {}),
          supportedFixes: outputDiagnostic?.supportedFixes || ['安全な出力パスへ戻して再試行してください'],
        },
      });
      return;
    }
    ...
    if (options.json) console.log(JSON.stringify(finalReceipt, null, 2));
      ...
      console.log(`architectureを比較しました ${outputPath}`);
      console.log(`${finalReceipt.validation.checksPassed}/${finalReceipt.validation.checkCount}件のチェック、完全性 ${finalReceipt.completeness}、${finalReceipt.proofLevel}、sha256 ${finalReceipt.artifact.sha256.slice(0, 12)}`);
      console.log(`レシート ${receiptPath}`);
    }
  } catch (error) {
    if (error instanceof ArchitectureDeltaError) {
      ...
      reportCompareFailure({ json: options.json, stage: 'internal', error: 'Architecture比較はコミット前に失敗しました。', code: 'delta/internal', details: { reason: error.message } });
    }
  } finally {
    try {
      fs.rmSync(stagingDirectory, { recursive: true, force: true });
    } catch (error) {
      console.error(`警告: compareのステージングディレクトリを削除できませんでした: ${error.message}`);
    }
  }
}
...
function sourceEvidenceFromArtifact(artifact) {
  ...
  if (evidence?.verified !== true || !evidence.repository?.url || !evidence.repository?.revision || !Number.isInteger(evidence.referenceCount)) {
    throw new Error('描画されたソース証拠レシートが不完全です。');
  }
  return evidence;
}
...
async function commandDeliver(args) {
  ...
  if (unknown.length) fail(`不明なdeliverオプションです: "${unknown[0]}"。`);
  ...
  try {
    ...
    reportDeliveryFailure({
      ...
      error: `deliver入力"${inputPath}"を読み取れませんでした: ${error.message}`,
      diagnostics: [repair],
    });
    return;
  }
  ...
  try {
    ...
    reportDeliveryFailure({
      ...
      diagnostics: error.archifyDiagnostics || [diagnostic({
        ...
        supportedFixes: ['安全な出力パスを指定して再試行してください'],
      })],
    });
    return;
  }
  ...
  try {
    ...
    const message = `deliverディレクトリ"${outputDirectory}"を作成できませんでした: ${error.message}`;
    reportDeliveryFailure({
      json,
      stage: 'prepare',
      type,
      ...
      diagnostics: [diagnostic({
        ...
        supportedFixes: ['書き込み可能な出力ディレクトリを指定してください'],
      })],
    });
    return;
  }
  ...
  try {
    ...
    const message = `"${outputPath}"と同じ場所にdeliver候補を作成できませんでした: ${error.message}`;
    reportDeliveryFailure({
      json,
      stage: 'prepare',
      type,
      ...
      diagnostics: [diagnostic({
        ...
        evidence: { ...(error?.code ? { systemCode: error.code } : {}), reason: error.message },
        supportedFixes: ['対象ファイルシステム上の書き込み可能な出力ディレクトリを指定してください'],
      })],
    });
    return;
  }
  ...
  try {
    try {
      ...
      const message = `deliver仕様を固定できませんでした: ${error.message}`;
      reportDeliveryFailure({
        json,
        stage: 'prepare',
        type,
    ...
    if (check.status !== 0) {
      ...
      reportDeliveryFailure({
        ...
        input: inputPath,
        output: outputPath,
        error: '最終成果物のチェックに失敗しました。以前の成果物は保持されています。',
    ...
    try {
      ...
      const message = `成功した成果物チェックのレシートを解析できませんでした: ${error.message}`;
      reportDeliveryFailure({
        json,
        stage: 'receipt',
        type,
    ...
    try {
      ...
      const message = `検証済みのdeliver候補を読み取れませんでした: ${error.message}`;
      reportDeliveryFailure({
        json,
        stage: 'receipt',
        type,
    ...
    try {
      ...
      const message = `リポジトリ証拠レシートを読み取れませんでした: ${error.message}`;
      reportDeliveryFailure({
        json,
        stage: 'receipt',
        type,
    ...
    try {
      ...
      reportDeliveryFailure({
        ...
        diagnostics: error.archifyDiagnostics || [diagnostic({
          ...
          subject: { output: outputPath },
          evidence: { ...(error?.code ? { systemCode: error.code } : {}) },
          supportedFixes: ['安全な出力パスへ戻して再試行してください'],
        })],
      });
      return;
    }
    ...
    try {
      ...
      const message = `検証済みのdeliver成果物"${outputPath}"をコミットできませんでした: ${error.message}`;
      reportDeliveryFailure({
        json,
        stage: 'commit',
        type,
        ...
        diagnostics: [diagnostic({
          ...
          subject: { output: outputPath },
          evidence: { ...(error?.code ? { systemCode: error.code } : {}), reason: error.message },
          supportedFixes: ['同じ書き込み可能なファイルシステム上にある、置換可能なファイルを出力先に指定してください'],
        })],
      });
      return;
    }
    ...
    if (open) {
      ...
      if (receipt.open.status !== 'opened') {
        console.error(`検証済みの成果物を開けませんでした（${receipt.open.status}）。手動で開いてください: ${outputPath}`);
      }
    }
    ...
    if (json) {
      ...
      console.log(`${type}をdeliverしました ${outputPath}`);
      const engineering = receipt.validation.engineeringProfile
        ? `、engineering ${receipt.validation.engineeringProfile}: pass`
        : '';
      console.log(`${receipt.validation.checksPassed}/${receipt.validation.checkCount}件の成果物チェック、composition ${receipt.validation.compositionProfile}: ${receipt.validation.compositionStatus}${engineering}、sha256 ${receipt.artifact.sha256.slice(0, 12)}`);
      if (receipt.open?.status === 'opened') console.log(`開きました ${outputPath}`);
    }
  } finally {
    try {
      fs.rmSync(stagingDirectory, { recursive: true, force: true });
    } catch (error) {
      console.error(`警告: deliverのステージングディレクトリ"${stagingDirectory}"を削除できませんでした: ${error.message}`);
    }
  }
}
...
async function commandPreview(args) {
  ...
  const unknown = repoArgs.rest.filter((arg) => arg.startsWith('--') && !knownOptions.has(arg));
  if (unknown.length) fail(`不明なpreviewオプションです: "${unknown[0]}"。`);
  const positional = repoArgs.rest.filter((arg) => !knownOptions.has(arg));
  ...
  try {
    ...
    fail(`ライブプレビューを読み込めませんでした: ${error.message}`, 1);
  }
  try {
    await runPreview({
      type,
    ...
    fail(`ライブプレビューを開始できませんでした: ${error.message}`, 1);
  }
}
...
async function commandVisualCheck(args) {
  ...
  if (unknown.length) fail(`不明なvisual-checkオプションです: "${unknown[0]}"。`, 1);
  ...
  try {
    ...
    fail(`visual-checkを読み込めませんでした: ${error.message}`, 1);
  }
  ...
  try {
    ...
    if (json) {
      ...
      console.error(`visual-checkに失敗しました: ${error.message}`);
    }
    process.exitCode = 1;
    return;
  }
  ...
  if (json) {
    ...
    console.log(`包含状態 ${result.receipt.containment.status}、キャプチャー ${result.receipt.captures.status}、視覚レビュー pending`);
    console.log(`レシート ${path.join(path.dirname(result.receipt.artifact.path), result.receipt.sidecars.receipt)}`);
    if (result.receipt.captures.contactSheet) {
      console.log(`コンタクトシート ${path.join(path.dirname(result.receipt.artifact.path), result.receipt.captures.contactSheet)}`);
    }
...
async function commandDoctor() {
  ...
  checks.push({
    label: `Node.js v${process.versions.node}（18以上が必要）`,
    ok: nodeMajor >= 18,
    missing: 0,
  ...
  checks.push({
    label: 'Architecture比較ランタイムと証明用fixture',
    ok: compareMissing === 0,
  ...
  for (const type of TYPES) {
    ...
    checks.push({
      label: `${type}のRenderer、スキーマ、サンプル`,
      ok: missing === 0,
      missing,
    });
  }
  ...
  const problems = [];
  if (nodeFailed) problems.push('Node.js 18以上が必要です');
  if (missingFiles) problems.push(`必須ファイルが${missingFiles}件不足しています`);
  if (invalidRuntime) problems.push(`ランタイムチェックが${invalidRuntime}件失敗しました`);
  console.error(`\nArchifyを使用する準備ができていません: ${problems.join('、')}。`);
  process.exitCode = 1;
}
...
async function commandGuide(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--json') {
      ...
      if (!['ja', 'en', 'zh'].includes(value)) fail('--langには"ja"、"en"、"zh"のいずれかを指定してください。');
      lang = value;
      index += 1;
  ...
  try {
    ...
    fail(`シナリオレシピガイドを読み込めませんでした: ${error.message}`, 1);
  }

  const query = queryParts.join(' ').trim();
  if (!query) {
    const selectedLang = lang || 'ja';
    if (json) {
      console.log(JSON.stringify({
        ok: true,
        mode: 'list',
...
async function commandBrands(args) {
  ...
  if (unknown.length) fail(`不明なbrandsオプションです: "${unknown[0]}"。`);
  const positional = args.filter((arg) => arg !== '--json');
  if (positional[0] === 'capture') {
    if (positional.length !== 2) fail('使用方法: archify brands capture <url> [--json]');
  ...
  if (json) {
    console.log(JSON.stringify({
      ...
      count: marks.length,
      marks,
      fallback: '"archify brands capture <url> --json"を実行し、返されたダイジェスト固定のbrand値を使用してください。',
    }, null, 2));
    return;
  }
  if (!marks.length) {
    console.log(`"${query}"に一致する組み込みブランドがありません。"archify brands capture <url> --json"を実行し、返されたダイジェスト固定のbrand値を使用してください。`);
    return;
  }
...
function commandDemo(args) {
  ...
  try {
    ...
    fail(`demoディレクトリ"${outputDirectory}"を作成できませんでした: ${error.message}`, 1);
  }
  ...
  console.log(`\nDemoを使用できます: ${output}`);
  console.log('次の手順: HTMLをブラウザーで開き、その後に独自の図を描画してください:');
...
function migrationPathDiagnostics(error, sourcePath, destinationPath) {
  ...
  return [diagnostic({
    ...
    message: 'workflowの移行元と移行先が異なるパスであることを確認できませんでした。',
    subject: { source: sourcePath, destination: destinationPath },
    evidence: {
      ...(error?.code ? { systemCode: error.code } : {}),
      reason: error?.message || String(error),
    },
    supportedFixes: ['安全でないパスの別名を削除するか、別の移行先パスを指定してください'],
  })];
}
  ...
  if (!ok) {
    ...
    if (!report.diagnostics.length) {
      report.diagnostics.push(diagnostic({
        ...
        message: '分類済みの診断がない状態でworkflowの移行に失敗しました。',
      }));
    }
...
function extractMigrationOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--to-schema') {
      ...
      if (!toSchema || toSchema.startsWith('--')) fail('--to-schemaにはスキーマバージョンが必要です。');
      index += 1;
      continue;
    }
    if (arg.startsWith('--to-schema=')) {
      toSchema = arg.slice('--to-schema='.length);
      if (!toSchema) fail('--to-schemaにはスキーマバージョンが必要です。');
      continue;
    }
    if (arg.startsWith('--')) fail(`不明なmigrateオプションです: "${arg}"。`);
    positional.push(arg);
  }
...
async function commandMigrate(args) {
  ...
  if (
    ...
    fail('使用方法: archify migrate workflow <old.json> <new.json> --to-schema 2 [--json]');
  }
  ...
  if (sourceDestinationAlias) {
    reportMigrationFailure({
      migrationDiagnostics: [diagnostic({
        ...
        message: 'workflowの移行元と移行先には異なるファイルを指定する必要があります。',
        subject: { source: sourcePath, destination: destinationPath },
        supportedFixes: ['移行元を変更せず、別の移行先パスを指定してください'],
      })],
    });
    return;
  }
  ...
  try {
    ...
    migration = {
      ...
      migrationDiagnostics: [diagnostic({
        code: 'migration/internal',
        message: 'workflowの移行が予期せず失敗しました。',
        evidence: { reason: error.message },
        supportedFixes: ['移行元のworkflowとこの診断をArchifyのメンテナーへ報告してください'],
      })],
    };
  }
  ...
  if (fs.existsSync(destinationPath) && !fs.lstatSync(destinationPath).isFile()) {
    reportMigrationFailure({
      ...
      migrationDiagnostics: [...migration.migrationDiagnostics, diagnostic({
        ...
        message: 'workflowの移行先には通常ファイルのパスを指定する必要があります。',
        subject: { destination: destinationPath },
        supportedFixes: ['存在しないパス、または通常ファイルを指すパスを移行先に指定してください'],
      })],
    });
    return;
  }
  ...
  try {
    ...
    reportMigrationFailure({
      ...
      migrationDiagnostics: [...migration.migrationDiagnostics, diagnostic({
        ...
        message: 'workflowの移行先を準備できませんでした。',
        subject: { destination: destinationPath },
        evidence: { ...(error?.code ? { systemCode: error.code } : {}), reason: error.message },
        supportedFixes: ['書き込み可能な移行先ディレクトリを指定してください'],
      })],
    });
    return;
  }
  ...
  try {
    ...
    if (pathsAlias(sourcePath, destinationPath)) {
      reportMigrationFailure({
        ...
        migrationDiagnostics: [...migration.migrationDiagnostics, diagnostic({
          code: 'migration/source-destination',
          message: 'コミット前に、workflowの移行元と移行先が同じファイルとして解決されました。',
          subject: { source: sourcePath, destination: destinationPath },
          supportedFixes: ['別の移行先パスを指定して再試行してください'],
        })],
      });
      return;
    }
    ...
    if (!currentSourceBytes.equals(sourceBytes)) {
      reportMigrationFailure({
        ...
        migrationDiagnostics: [...migration.migrationDiagnostics, diagnostic({
          ...
          message: '移行先の検証中にworkflowの移行元が変更されました。',
          subject: { source: sourcePath },
          supportedFixes: ['変更されていないworkflowの移行元ファイルを使って再試行してください'],
        })],
      });
      return;
    }
    ...
    if (options.json) console.log(JSON.stringify(report, null, 2));
      ...
      console.log(`workflowスキーマをv1からv2へ移行しました: ${sourcePath} → ${destinationPath}`);
    } else {
      console.log(`workflowスキーマv2の移行を検証しました: ${sourcePath} → ${destinationPath}`);
    }
  } catch (error) {
    const migrationDiagnostics = Array.isArray(error?.archifyDiagnostics)
      ...
      : [diagnostic({
        ...
        message: '検証済みのworkflow移行をコミットできませんでした。',
        subject: { destination: destinationPath },
        evidence: { ...(error?.code ? { systemCode: error.code } : {}), reason: error.message },
        supportedFixes: ['書き込み可能な通常ファイルを移行先に指定して再試行してください'],
      })];
    reportMigrationFailure({
      ...migration,
    ...
    try {
      ...
      console.error(`警告: workflow移行のステージングディレクトリ"${stagingDirectory}"を削除できませんでした: ${error.message}`);
    }
  }
}
...
function commandValidate(args) {
  ...
  const unknown = args.filter((arg) => arg.startsWith('--') && !knownOptions.has(arg));
  if (unknown.length) fail(`不明なvalidateオプションです: "${unknown[0]}"。`);
  const json = args.includes('--json');
  ...
  if (layoutJson) {
    if (!['architecture', 'workflow'].includes(type)) {
      fail('--layout-jsonは現在architecture図とworkflow図でのみ使用できます。');
    }
    ...
    if (render.status !== 0) {
      ...
      if (check.status !== 0) {
        ...
        try {
          ...
          checker = { ok: false, diagnostic: '成果物チェッカーが解析可能なレシートを出力せずに失敗しました。' };
        }
        reportValidateFailure({
          json,
          stage: 'check',
        ...
        if (json) {
          ...
          const engineering = engineeringProfile
            ...
            : '';
          console.log(`ok ${type} ${path.resolve(input)}（${result.checks.length}件の成果物チェック、composition ${result.composition.profile}: エラー${result.composition.summary.errors}件、警告${result.composition.summary.warnings}件${engineering}）`);
        }
      }
    }
  } finally {
...
switch (command) {
  ...
  case 'inspect':
    if (args[0] !== 'architecture') {
      fail('inspectは現在architecture図でのみ使用できます。');
    }
