...
function requireComparableShape(diagram, side) {
  if (diagram?.schema_version !== 1) {
    fail('delta/schema-version-mismatch', `${side} は schema_version 1 を使用する必要があります。`, { side, path: '/schema_version', actual: diagram?.schema_version });
  }
  if (diagram?.diagram_type !== 'architecture') {
    fail('delta/type-mismatch', `${side} は diagram_type architecture を使用する必要があります。`, { side, path: '/diagram_type', actual: diagram?.diagram_type });
  }
}
...
function stableIndex(items, collection, side, missingCode = 'delta/stable-id-required') {
  ...
  if (missing.length) {
    fail(missingCode, `${side} の ${collection} を比較するには、作成時に安定した id を指定する必要があります。`, {
      side,
      paths: sorted(missing),
      supportedFixes: [`${collection} の各項目に一意な id を追加する`],
    });
  }
  if (duplicates.length) {
    fail('delta/duplicate-stable-id', `${side} の ${collection} に重複する id があります。`, {
      side,
      collection,
      ids: sorted(new Set(duplicates)),
      supportedFixes: [`${collection} のすべての id を一意にする`],
    });
  }
  return index;
}
...
function boundaryIndex(boundaries, side) {
  ...
  if (ambiguous.length) {
    fail('delta/boundary-key-ambiguous', `${side} の境界の kind + label キーは一意である必要があります。`, {
      side,
      boundaries: sorted(new Set(ambiguous)),
      supportedFixes: ['重複する境界の一方を改名するか、将来のスキーマバージョンで安定した境界 id を追加する'],
    });
  }
  return index;
}
...
export function compareArchitecture(base, head, evidence = {}) {
  ...
  if (!shared.length) {
    fail('delta/no-shared-component-id', 'スナップショット間で共通するコンポーネント id がないため、同じシステムを表していることを Archify が証明できません。', {
      supportedFixes: ['スナップショット間で作成済みコンポーネント id を少なくとも1つ維持する'],
    });
  }
  ...
  if (baseRepository && headRepository && baseRepository.url !== headRepository.url) {
    fail('delta/repository-mismatch', 'スナップショットが異なるリポジトリを指定しています。', {
      baseRepository: baseRepository.url,
      headRepository: headRepository.url,
      supportedFixes: ['同じリポジトリのスナップショットを比較するか、両方の入力からリポジトリ証拠を削除する'],
    });
  }
  ...
  return {
    ...
    limitations: [
      '作成済みアーキテクチャ IR のみを対象とし、実行時の影響、因果関係、リスク、マージ可否は推測しません。',
      '境界の同一性は kind + label から保守的に導出されます。',
    ],
  };
}
...
export function extractArchitectureSvg(html) {
  ...
  if (!match) fail('delta/svg-missing', '検証済みアーキテクチャ成果物に主要 SVG が含まれていません。');
  return match[0];
}

export function extractArtifactCss(html) {
  const match = html.match(/<style>([\s\S]*?)<\/style>/);
  if (!match) fail('delta/css-missing', '検証済みアーキテクチャ成果物にスタイルシートが含まれていません。');
  return match[1];
}
...
export function architectureDeltaChangeRows(receipt) {
  const rows = [];
  for (const change of receipt.changes.components) rows.push({ ...change, kind: 'コンポーネント', kindKey: 'component', key: `component:${change.id}`, id: change.id });
  for (const change of receipt.changes.connections) rows.push({ ...change, kind: '関係', kindKey: 'relationship', key: `relationship:${change.id}`, id: change.id });
  for (const change of receipt.changes.boundaries) rows.push({ ...change, kind: '境界', kindKey: 'boundary', key: `boundary:${change.key}`, id: change.key });
...
export function renderArchitectureDeltaHtml({ receipt, baseSvg, deltaSvg, headSvg, baseHtml = '', headHtml = '', artifactCss }) {
  ...
  const proof = receipt.proofLevel === 'revision-pinned' ? 'リビジョン固定済み入力' : '作成済みスナップショット';
  ...
  }).join('\n') : '<li class="empty">作成済みアーキテクチャに変更はありません。</li>';
  const baseView = baseHtml
    ? `<iframe class="snapshot-frame" title="変更前のアーキテクチャエクスプローラー" srcdoc="${esc(baseHtml)}"></iframe>`
    : baseSvg;
  const headView = headHtml
    ? `<iframe class="snapshot-frame" title="変更後のアーキテクチャエクスプローラー" srcdoc="${esc(headHtml)}"></iframe>`
    : headSvg;
  const html = `<!doctype html>
<html lang="ja" data-theme="dark" data-preset="${esc(receipt.view.visualPreset)}">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(receipt.head.title)} アーキテクチャ差分</title>
<style>
${artifactCss}
...
<body><main class="proof-page"><header class="proof-head"><div><p class="eyebrow">アーキテクチャ差分 · ${proof}</p><h1>マージする前に<br>変更点を確認。</h1><p class="subtitle">${esc(receipt.base.title)} → ${esc(receipt.head.title)}</p></div><div class="metrics"><div class="metric add"><strong>${total(receipt.summary, 'added')}</strong><span>追加</span></div><div class="metric remove"><strong>${total(receipt.summary, 'removed')}</strong><span>削除</span></div><div class="metric change"><strong>${changed}</strong><span>変更</span></div></div></header>
<div class="proof-tools"><div class="view-switch" role="tablist" aria-label="アーキテクチャのスナップショット"><button role="tab" data-target="base" aria-selected="false">変更前</button><button role="tab" data-target="delta" aria-selected="true">差分</button><button role="tab" data-target="head" aria-selected="false">変更後</button></div><div class="legend"><span class="add"><i></i>+ 追加</span><span class="remove"><i></i>− 削除</span><span class="change"><i></i>~ 変更</span><span class="move"><i></i>↔ 移動</span></div><div><button class="utility" id="export-svg" type="button">SVG を書き出す</button> <button class="utility" id="share-card" type="button">共有カード</button> <button class="utility" id="preset" type="button">プリセット</button> <button class="utility" id="theme" type="button">テーマ</button></div></div>
<nav class="review-strip" aria-label="作成済み変更のレビュー"><button class="review-step" id="review-overview" type="button" disabled>概要</button><button class="review-step" id="review-previous" type="button" aria-label="前の作成済み変更" disabled>←</button><button class="review-step" id="review-play" type="button" aria-pressed="false"${rows.length ? '' : ' disabled'}>レビュー</button><button class="review-step" id="review-next" type="button" aria-label="次の作成済み変更" disabled>→</button><div class="review-status" id="review-status" role="status" aria-live="polite">概要 · 作成済み変更 ${rows.length} 件</div></nav>
...
<details${rows.length <= 10 ? ' open' : ''}><summary>作成済み変更の正確な一覧 · ${rows.length} 件</summary><ul class="changes">${rowHtml}</ul></details>
<footer class="proof-foot"><span>安定した ID のみ · 完全性: 完全 · ${proof}</span><span>作成済み IR のみ · リスクやマージ可否は推測しません</span></footer></main>
...
<script>(()=>{
  ...
  function stopPlayback() {
    ...
    playButton.textContent = activeIndex === rowButtons.length - 1 && rowButtons.length ? '再生' : 'レビュー';
  ...
  function failReview() {
    ...
    status.textContent = 'レビューを利用できません · 比較対象の識別情報が一致しません';
  }
  ...
  function overview() {
    ...
    status.textContent = '概要 · 作成済み変更 ' + rowButtons.length + ' 件';
    updateControls();
  }
  ...
  function canonicalDeltaSvg() {
    ...
    if (!source) throw new Error('正規の差分 SVG を利用できません。');
  ...
  function loadSvgImage(blob) {
    return new Promise((resolve, reject) => {
      ...
      image.onerror = () => { URL.revokeObjectURL(url); reject(new Error('正規の差分 SVG をラスター化できませんでした。')); };
      image.src = url;
    });
  }
  ...
  function pngBlob(canvas) {
    return new Promise((resolve, reject) => canvas.toBlob((blob) => {
      ...
      else reject(new Error('Canvas から共有カードの PNG が返されませんでした。'));
    }, 'image/png'));
  }
  ...
  async function shareCard() {
    ...
    if (!ctx) throw new Error('アーキテクチャ差分共有カード用の 2D Canvas コンテキストを利用できません。');
    ...
    ctx.fillText('アーキテクチャ差分', 66, 68);
    ctx.font = '700 34px ui-monospace, SFMono-Regular, Menlo, monospace';
    ctx.fillStyle = '#e6edf5';
    const title = String(receipt.head.title || 'アーキテクチャ').slice(0, 46) + ' アーキテクチャ差分';
    ctx.fillText(title.slice(0, 58), 66, 112);
    ctx.font = '700 17px ui-monospace, SFMono-Regular, Menlo, monospace';
    ctx.fillStyle = '#8aa0b5';
    const componentLine = 'コンポーネント  +' + receipt.summary.components.added + '  ~' + receipt.summary.components.changed + '  −' + receipt.summary.components.removed;
    const connectionLine = '関係  +' + receipt.summary.connections.added + '  ~' + receipt.summary.connections.changed + '  −' + receipt.summary.connections.removed;
    const boundaryLine = '境界スコープ  +' + receipt.summary.boundaries.added + '  ~' + receipt.summary.boundaries.changed + '  −' + receipt.summary.boundaries.removed;
    ...
    const movementSummary = '↔ 移動 ' + receipt.summary.components.moved + ' · 経路変更 ' + receipt.summary.connections.rerouted + ' · 表示 ' + (receipt.summary.presentationChanged ? '変更あり' : '変更なし');
    const secondary = authoredChanges === 0
      ? '作成済みアーキテクチャに変更なし · ' + movementSummary
      : movementSummary;
    ctx.fillText(secondary, 66, 181);
    const proofLine = receipt.proofLevel === 'revision-pinned'
      ? 'REV ' + String(receipt.base.revision).slice(0, 8) + ' → ' + String(receipt.head.revision).slice(0, 8) + ' · リビジョン固定済み入力'
      : '作成済みスナップショット';
    ctx.textAlign = 'right';
    ...
    ctx.fillStyle = '#8aa0b5';
    ctx.fillText('安定した作成済み ID · 静的で完全な差分 · リスクやマージ可否は推測しません', 66, 588);
  ...
  window.Archify.exportMenu = {
    ...
    run(format) {
      ...
      throw new Error('不明なアーキテクチャ差分の書き出し形式です: ' + format);
    },
  };
...
export function validateArchitectureDeltaHtml(html, receipt) {
  ...
  if (!['base', 'delta', 'head'].every((id) => (html.match(new RegExp(`<section class="canvas" data-view="${id}"`, 'g')) || []).length === 1)) failures.push('変更前、差分、変更後のキャンバスがそれぞれ1つ必要です');
  if ((html.match(/class="snapshot-frame" title="変更前のアーキテクチャエクスプローラー"/g) || []).length !== 1
    || (html.match(/class="snapshot-frame" title="変更後のアーキテクチャエクスプローラー"/g) || []).length !== 1) {
    failures.push('変更前と変更後には、それぞれ完全なアーキテクチャエクスプローラーを維持する必要があります');
  }
  if (!svgBalanced || svgDepth !== 0 || svgRoots !== 1 || deltaMarkup.slice(0, rootStart).trim() || deltaMarkup.slice(rootEnd).trim()) failures.push('差分キャンバスにはルート SVG がちょうど1つ必要です');
  if ((html.match(/id="archify-compare-receipt"/g) || []).length !== 1) failures.push('埋め込み比較レシートがちょうど1つ必要です');
  if (!html.includes('aria-label="作成済み変更のレビュー"')) failures.push('正確な ID を使用する変更ナビゲーターがありません');
  if ((html.match(/class="change-row"/g) || []).length !== rows.length) failures.push('変更ナビゲーターの行数がレシートと一致しません');
  if (!html.includes('id="export-svg"') || !html.includes('id="share-card"')
    ...
    failures.push('正規の差分 SVG または共有カードの書き出し契約がありません');
  }
  for (const [index, row] of rows.entries()) {
    ...
    if (rowMatches.length !== 1) failures.push(`変更行 ${row.key} がちょうど1つ必要です`);
    const targets = reviewTargetTags(deltaMarkup, row);
    if (!targets.length) failures.push(`差分の識別情報 ${row.key} がありません`);
    if (targets.some(({ tag }) => !/\bdata-delta-state="[^"]+"/.test(tag))) failures.push(`差分対象の状態 ${row.key} がありません`);
    ...
    if (!signature || signature !== expectedSignature || storedSignature !== expectedSignature) failures.push(`差分対象の署名 ${row.key} が曖昧です`);
    if (rowMatches[0]?.match(/\bdata-change-index="([^"]+)"/)?.[1] !== String(index)) failures.push(`変更行 ${row.key} の順序が正しくありません`);
    ...
    if (JSON.stringify(states) !== JSON.stringify(reviewPrimaryStates(row).sort())) failures.push(`差分の識別情報 ${row.key} が曖昧です`);
    const classifications = row.classifications.join(',');
    if (primary.some((tag) => tag.match(/\bdata-delta-classifications="([^"]*)"/)?.[1] !== classifications)) failures.push(`差分分類 ${row.key} が競合しています`);
  }
  ...
  if (/\b(?:SAFE|LOW RISK|MERGEABLE|NO IMPACT|VERIFIED PR)\b/i.test(deltaShell)) failures.push('禁止されているリスクまたはマージ可否の主張が含まれています');
  if (/\b(?:NaN|Infinity)\b/.test(html)) failures.push('有限でない出力が含まれています');
  if (receipt.completeness !== 'complete') failures.push('レシートが完全ではありません');
  if (failures.length) fail('delta/artifact-invalid', `アーキテクチャ差分成果物の検証に失敗しました: ${failures.join('; ')}。`, { failures });
