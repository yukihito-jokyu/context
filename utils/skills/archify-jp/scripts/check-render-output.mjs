...
if (!input || input === '-h' || input === '--help') {
  console.error('使用方法: node scripts/check-render-output.mjs <diagram.html>');
...
if (svgMatches.length === 1) {
  ...
  addCheck(
    'orthogonal_arrows',
    diagonal.length === 0,
    diagonal.map(({ arrow, segmentIndex }) => `${arrow.kind} ${arrow.index}の区間${segmentIndex + 1}: ${arrow.raw}`),
  );
  ...
  addCheck(
    ...
    !labelClearanceIsError || labelRouteClearance.length === 0,
    labelRouteClearance.map((hit) => (
      `[composition/label-route-clearance] ${qualityProfile} ${relationshipName(hit.labelRelation)}上のラベル「${hit.label?.label || hit.labelRelation?.label || ''}」は、${relationshipName(hit.otherRelation)}の区間${hit.segmentIndex} [${formatPoint(hit.start)}] -> [${formatPoint(hit.end)}]から${Math.round(hit.clearance * 10) / 10}pxです${hit.intersectionLength > 0 ? `（マスクによって${Math.round(hit.intersectionLength * 10) / 10}px隠れています）` : ''}（最小${hit.threshold}px）。レンダラーが対応するラベル制御（sequenceではmessage y、それ以外ではlabelAt、labelDx、labelDy、labelSegment）を使用するか、もう一方の関係のroute／via／channelを調整してください。`
    )),
  );
  addCheck(
    ...
    !crossingIsError || relationshipCrossings.length === 0,
    relationshipCrossings.map((hit) => (
      `[composition/proper-crossing] ${qualityProfile} ${relationshipName(hit.left)}が[${formatPoint(hit.point)}]で${relationshipName(hit.right)}と交差しています`
    )),
  );
  addCheck(
    ...
    !corridorIsError || ambiguousCorridors.length === 0,
    ambiguousCorridors.map((hit) => (
      `[composition/ambiguous-corridor] ${qualityProfile} ${relationshipName(hit.left.relation)}が[${formatPoint(hit.overlapStart)}] -> [${formatPoint(hit.overlapEnd)}]で${relationshipName(hit.right.relation)}と${Math.round(hit.overlapLength * 10) / 10}pxの経路を共有しています`
    )),
  );
  addCheck(
    ...
    containerBorderRuns.map((hit) => (
      `[composition/container-border-run] ${relationshipName(hit.relation)}の区間${hit.segmentIndex} [${formatPoint(hit.overlapStart)}] -> [${formatPoint(hit.overlapEnd)}]が、${frameName(hit.frame)}の${hit.side}境界に${Math.round(hit.overlapLength * 10) / 10}px沿っています`
    )),
  );
  addCheck(
    'route_rhythm',
    !rhythmIsError || routeRhythmIssues.length === 0,
    routeRhythmIssues.map((hit) => (
      `[${hit.code}] ${qualityProfile} ${relationshipName(hit.relation)}に${Math.round(hit.length * 10) / 10}pxの${hit.position}区間${hit.segmentIndex} [${formatPoint(hit.start)}] -> [${formatPoint(hit.end)}]があります`
    )),
  );
  ...
  if (legendStart >= 0) {
    ...
    addCheck(
      'legend_clearance',
      collisions.length === 0,
      collisions.map((hit) => `${hit.arrow.kind} ${hit.arrow.index}が凡例${hit.box.label}と交差しています`),
    );
  } else {
    addCheck('legend_clearance', true, ['凡例マーカーは見つかりませんでした']);
  }
}
...
function relationshipName(arrow) {
  return arrow.id
    ? `関係ID「${arrow.id}」（「${arrow.from}」->「${arrow.to}」）`
    : `関係「${arrow.from}」->「${arrow.to}」`;
}
