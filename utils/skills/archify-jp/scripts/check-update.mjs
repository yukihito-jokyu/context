...
function assertBoundedRegularFile(metadata, maxBytes) {
  if (!metadata.isFile()
    ...
    throw new Error('安全でないJSONファイルです');
  }
}
...
async function readJsonFile(target, maxBytes, expectedMetadata = null) {
  ...
  if (expectedMetadata && !isSameFile(expectedMetadata, pathMetadata)) {
    throw new FileIdentityChangedError('JSONファイルの識別情報が変更されました');
  }
  ...
  try {
    ...
    if (!isSameFile(pathMetadata, handleMetadata)
      ...
      throw new FileIdentityChangedError('JSONファイルを開いている間に変更されました');
    }
    ...
    if (size > maxBytes) throw new Error('JSONファイルが大きすぎます');
...
function assertSafeDirectory(metadata) {
  if (!metadata.isDirectory() || metadata.isSymbolicLink()) {
    throw new Error('安全でないキャッシュディレクトリです');
  }
}
...
async function verifyDirectorySnapshots(token, snapshots, description) {
  if (!token.trusted) throw new CachePathChangedError('キャッシュディレクトリは信頼できる状態ではなくなりました');
  try {
    for (const snapshot of snapshots) {
      ...
      if (!isSameFile(snapshot.metadata, metadata)) {
        throw new Error(`${description}が変更されました`);
      }
    }
  } catch (error) {
    throw invalidateCacheToken(token, `${description}が変更されました`, error);
  }
}

async function verifyCacheToken(token) {
  await verifyDirectorySnapshots(token, token.ancestorSnapshots, 'キャッシュディレクトリ');
}
...
function cacheTokenFor(cacheDirectory) {
  ...
  if (!token || !token.trusted) {
    throw new CachePathChangedError('キャッシュディレクトリが安全に準備されていません');
  }
  return token;
}
...
function resolveCacheTarget(token, target) {
  ...
  if (resolved === token.directory || !isWithinDirectory(token.directory, resolved)) {
    throw invalidateCacheToken(token, 'キャッシュ変更が準備済みディレクトリの外へ逸脱しました');
  }
  return resolved;
}
...
async function captureMutationParentSnapshots(token, targets) {
  ...
  try {
    ...
    throw invalidateCacheToken(token, 'キャッシュ変更先の親が変更されました', error);
  }
...
async function verifyMutationParentSnapshots(token, snapshots) {
  try {
    for (const snapshot of snapshots) {
      ...
      assertSafeDirectory(metadata);
      if (!isSameFile(snapshot.metadata, metadata)) {
        throw new Error('キャッシュ変更先の親が変更されました');
      }
    }
  } catch (error) {
    const raced = new CacheOperationRaceError('キャッシュ変更先の親が変更されました');
    raced.cause = error;
    throw raced;
  }
}
...
async function stableCacheEntry(
  ...
  try {
    ...
    if (expectedMetadata && !isSameFile(expectedMetadata, first)) {
      throw new Error('キャッシュエントリの識別情報が変更されました');
    }
    await verifyCacheToken(token);
    ...
    throw invalidateCacheToken(token, 'キャッシュ変更によって信頼できないオブジェクトが生成されました', error);
  }
}
...
async function stablePreparedDirectory(token, target) {
  try {
    ...
    if (!isSameFile(first, second)) throw new Error('準備済みディレクトリの識別情報が変更されました');
    return second;
  } catch (error) {
    throw invalidateCacheToken(token, '準備済みディレクトリを検証できませんでした', error);
  }
}
...
async function assertCacheTargetAbsent(token, target) {
  ...
  try {
    ...
    throw invalidateCacheToken(token, 'キャッシュ変更対象を検証できませんでした', error);
  }
  throw invalidateCacheToken(token, 'キャッシュ変更後に予期しないオブジェクトが残りました');
}
...
async function guardedCacheMutation(token, parentSnapshots, {
  ...
  try {
    ...
    throw invalidateCacheToken(token, 'キャッシュ変更を検証できませんでした', error);
  }
}
...
async function cacheWriteFile(cacheDirectory, target, data, options) {
  if (options?.flag !== 'wx') {
    throw new Error('キャッシュファイルは排他的に作成する必要があります');
  }
  ...
  return guardedCacheMutation(token, parents, {
    mutate: async () => {
      ...
      try {
        ...
        if (!openedMetadata.isFile() || openedMetadata.isSymbolicLink()) {
          throw new Error('キャッシュ書き込みで通常ファイルを開けませんでした');
        }
        ...
        if (!isSameFile(openedMetadata, writtenMetadata)
          ...
          throw new Error('キャッシュ書き込みで開いたファイルの識別情報とサイズが維持されませんでした');
        }
    ...
    verifyAfter: (writtenMetadata) => stableCacheEntry(token, resolved, (metadata) => {
      if (!metadata.isFile() || metadata.isSymbolicLink()) {
        throw new Error('キャッシュ書き込みで通常ファイルが作成されませんでした');
      }
    }, writtenMetadata),
  });
}
...
async function cacheRename(
  ...
  if (typeof validate !== 'function') {
    throw new TypeError('キャッシュの名前変更には明示的なエントリバリデーターが必要です');
  }
  const token = cacheTokenFor(cacheDirectory);
...
async function cacheRm(cacheDirectory, target, options) {
  if (options?.recursive) {
    throw new Error('キャッシュの再帰的削除は許可されていません');
  }
  const token = cacheTokenFor(cacheDirectory);
  const resolved = resolveCacheTarget(token, target);
  const parents = await captureMutationParentSnapshots(token, [resolved]);
  ...
  try {
    ...
    throw invalidateCacheToken(token, 'キャッシュの削除対象が安全ではありません', error);
  }
      ...
      return entries.flatMap((entry) => {
        ...
        if (!entry.isDirectory() || entry.isSymbolicLink()) {
          throw invalidateCacheToken(
            token,
            `キャッシュ操作${entry.name}は安全なディレクトリではありません`,
          );
        }
...
async function reserveOperation(cacheDirectory) {
  ...
  while (true) {
    ...
    if (generation > MAX_OPERATION_GENERATION) throw new Error('操作世代の空間を使い切りました');
    ...
    try {
      ...
      try {
        ...
        if (await operationWasFenced(cacheDirectory, operation)) {
          const fencedError = new OperationFencedError('予約中に操作がフェンスされました');
...
function notification(localRelease, candidate) {
  return {
    ...
    summary: `Archify ${candidate.version}を利用できます。詳細は公式リリースノートを確認してください。`,
...
async function readBoundedBody(response) {
  ...
  if (Number.isFinite(contentLength) && contentLength > MAX_RESPONSE_BYTES) {
    ...
    throw new UpdateContractError('マニフェストのレスポンスが大きすぎます');
  }
  if (!response.body || typeof response.body.getReader !== 'function') {
    throw new UpdateContractError('マニフェストのレスポンスに上限付きストリームがありません');
  }
  ...
  try {
    ...
    throw new UpdateContractError('マニフェストが有効なUTF-8ではありません');
  }
}
...
async function fetchCandidate({ fetchImpl, manifestUrl, timeoutMs }) {
  if (manifestUrl !== DEFAULT_MANIFEST_URL) throw new UpdateContractError('予期しないマニフェストURLです');
  const controller = new AbortController();
  let timer;
  const timeout = new Promise((_resolve, reject) => {
    timer = setTimeout(() => {
      controller.abort();
      reject(new Error('更新確認がタイムアウトしました'));
    }, timeoutMs);
  });
  const request = (async () => {
    ...
    if (response.status !== 200) {
      await cancelResponseBody(response);
      throw new Error(`マニフェストがHTTP ${response.status}を返しました`);
    }
    const mediaType = response.headers.get('content-type')?.split(';', 1)[0].trim().toLowerCase();
    if (mediaType !== 'application/json') {
      await cancelResponseBody(response);
      throw new UpdateContractError('マニフェストがJSONではありません');
    }
    ...
    try {
      ...
      throw new UpdateContractError('マニフェストが有効な上限付きJSONではありません');
    }
