...
export function parseSemver(value) {
  if (typeof value !== 'string' || value.length > 128) {
    throw new UpdateContractError(`SemVerが無効です: ${JSON.stringify(value)}`);
  }
  const match = SEMVER.exec(value);
  if (!match) throw new UpdateContractError(`SemVerが無効です: ${JSON.stringify(value)}`);
...
export function validateCanonicalUtcTimestamp(value) {
  if (typeof value !== 'string' || !UTC_SECONDS.test(value)) {
    throw new UpdateContractError('公開日時はYYYY-MM-DDTHH:mm:ssZ形式で指定してください');
  }
  const timestamp = Date.parse(value);
  if (!Number.isFinite(timestamp)
    || new Date(timestamp).toISOString().replace('.000Z', 'Z') !== value) {
    throw new UpdateContractError('公開日時が実在するUTC日時ではありません');
  }
  return value;
}
...
export function validateLocalRelease(value) {
  if (!hasExactKeys(value, [
    ...
    throw new UpdateContractError('ローカルリリース識別情報が無効です');
  }
  const expectedChannel = releaseChannelForVersion(value.version);
  if (value.channel !== expectedChannel) {
    throw new UpdateContractError('ローカルリリースのチャンネルがバージョンと一致しません');
  }
  return {
...
export function validateReleaseNotesUrl(value, version) {
  if (!isStableCoreVersion(version)) {
    throw new UpdateContractError('リリースノートには安定版のコアバージョンが必要です');
  }
  const expected = `https://github.com/tt-a1i/archify/releases/tag/v${version}`;
  if (value !== expected) {
    throw new UpdateContractError('リリースノートURLが正確な信頼済みリリースパスの外部を指しています');
  }
  return value;
}
...
export function validateStableUpdateManifest(value) {
  if (!hasExactKeys(value, [
    ...
    throw new UpdateContractError('変更不能な安定版リリース識別情報が無効です');
  }
