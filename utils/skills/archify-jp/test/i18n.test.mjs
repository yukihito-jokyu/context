...
function authoredExample(type, locale) {
  ...
  const nextAuthoredText = () => {
    ...
    const value = locale === 'zh-CN'
      ...
      : (locale === 'ja'
        ? `文言${String(authoredIndex).padStart(2, '0')}`
...
test('zh-CN localizes renderer-owned output across all five modes without translating authored content', () => {
  assert.deepEqual(SUPPORTED_LOCALES, ['en', 'ja', 'zh-CN']);
...
test('explicit en, ja, and zh-CN preserve complete authored field inventories across all five modes', () => {
  for (const type of Object.keys(EXAMPLES)) {
    ...
    for (const candidate of [english, japanese, chinese]) {
      ...
      if (locale === 'zh-CN') {
        ...
        assert.ok(result.html.includes(`<title>${candidate.document.meta.title} 図</title>`), type);
...
test('omitted locale preserves authored content and uses the Japanese Viewer contract in all five modes', () => {
  for (const type of Object.keys(EXAMPLES)) {
    ...
    assert.equal(result.status, 0, `${type}: ${result.stderr || result.stdout}`);
    assert.match(result.html, /^<!DOCTYPE html>\n<html lang="ja"/);
    assert.ok(result.html.includes(`<title>${authoredTitle} 図</title>`), `${type}: authored title changed`);
    assert.ok(result.html.includes(`<h1>${authoredTitle}</h1>`), `${type}: authored heading changed`);
