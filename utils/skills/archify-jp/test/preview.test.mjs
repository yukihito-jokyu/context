...
test('preview: invalid candidates preserve the last verified artifact and repair automatically', { timeout: 30000 }, async () => {
  ...
  try {
    ...
    assert.match(failed.failure.message, /deliver入力.*を読み取れませんでした/);
    ...
    assert.match(page.body, /Archify ライブプレビュー/);
    assert.match(page.body, /<summary role="button" aria-controls="diagnostic-panel">診断を表示<\/summary>/);
