...
test('deployment ownership profile reports exact owners, scopes, state, and crossing mechanisms', () => {
  ...
  candidate.boundaries.find((boundary) => boundary.label === 'プライベートアプリケーションネットワーク').wraps =
    candidate.boundaries.find((boundary) => boundary.label === 'プライベートアプリケーションネットワーク').wraps.filter((id) => id !== 'redis');
  ...
  assert.deepEqual(boundaryDiagnostic.supportedFixes, [
    `/connections/${boundaryDiagnostic.subject.index}/labelに実際の境界間の仕組みを設定する`,
  ]);
});
