...
test('cli: help lists commands and diagram types', () => {
  ...
  assert.match(result.stdout, /architecture, workflow, sequence, dataflow, lifecycle, class, er/);
});
...
test('cli: doctor reports a complete installation is ready', () => {
  ...
  assert.match(result.stdout, /\[ok\] classのRenderer、スキーマ、サンプル/);
  assert.match(result.stdout, /\[ok\] erのRenderer、スキーマ、サンプル/);
