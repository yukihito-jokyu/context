const RAW_RECIPES = [
  {
    ...
    start: {
      ...
      ja: { descriptionPrompt: 'Archifyを使い、次の自然言語によるシステム説明を高レベルのアーキテクチャ図にしてください: [ユーザー、主要コンポーネント、主経路、外部依存関係、境界を記述]。リポジトリは不要です。図を実質的に変える不足情報だけを質問し、残る不明点は推測せず明示してください。8〜12個の主要コンポーネントを使い、ひと目で分かる主経路を1つ維持してください。' },
    },
    signals: [['system overview', 12], ['architecture', 10], ['components', 6], ['services', 4], ['repository', 5], ['trust boundary', 8], ['架构', 10], ['系统总览', 12], ['组件', 6], ['服务', 4], ['仓库', 5], ['信任边界', 8], ['システム概要', 12], ['アーキテクチャ', 10], ['コンポーネント', 6], ['サービス', 4], ['リポジトリ', 5], ['信頼境界', 8]],
    en: {
    ...
    ja: {
      title: 'システム概要', question: '何が存在し、誰が担当し、どのようにつながっていますか？',
      summary: '主要コンポーネント、外部依存関係、主経路、信頼境界を範囲の明確な図で示します。',
      useWhen: 'オンボーディング、設計レビュー、リポジトリの把握、サービス全体像の説明に使用します。',
      avoidWhen: '正確な呼び出し順序、状態遷移、行レベルのデータリネージが必要な場合は使用しません。',
      include: ['8〜12個の主要コンポーネント', '1つの主経路', '外部依存関係', '担当範囲または信頼境界'],
      prompt: 'このリポジトリを分析し、Archifyを使って高レベルのアーキテクチャ図を作成してください。8〜12個の主要なランタイムコンポーネント、1つの主要なリクエストまたはデータ経路、外部依存関係、担当範囲または信頼境界を示してください。補足情報はエッジを増やさずカードに配置してください。',
    },
  },
  {
    ...
    signals: [['deployment topology', 14], ['region', 7], ['vpc', 9], ['cluster', 6], ['availability zone', 8], ['ownership', 7], ['cloud deployment', 12], ['部署拓扑', 14], ['区域', 6], ['集群', 6], ['可用区', 8], ['资源归属', 9], ['跨区', 8], ['デプロイトポロジー', 14], ['リージョン', 7], ['VPC', 9], ['クラスター', 6], ['アベイラビリティゾーン', 8], ['担当範囲', 7], ['クラウドデプロイ', 12]],
    en: {
    ...
    ja: {
      title: 'デプロイと担当範囲', question: '各ワークロードはどこで実行され、何が境界を越えますか？',
      summary: 'リージョン、ネットワーク、クラスター、ワークロード、ストア、境界を越える仕組みに焦点を当てたデプロイ図です。',
      useWhen: 'クラウドレビュー、本番準備、マルチリージョン計画、インフラ担当の引き継ぎに使用します。',
      avoidWhen: 'デプロイの事実が不明な場合や、配置ではなくアプリケーションの動作が主題の場合は使用しません。',
      include: ['リージョンとネットワーク', 'ワークロードの担当範囲', 'ステートフルサービス', '名前を明示した境界通過'],
      prompt: 'Archifyを使って本番デプロイのトポロジーを描いてください。リソースをリージョン、ネットワーク、クラスター、担当者ごとにまとめ、ワークロードとステートフルサービスを示し、境界を越えるすべての仕組みにラベルを付けてください。デプロイの事実を推測せず、不明な領域は明示してください。ユーザーがフェイルクローズのデプロイレビューを望む場合は、meta.engineering_profileをdeployment-ownershipに設定する前に確認し、それ以外の場合はengineering profileを設定しないでください。',
    },
  },
  {
    ...
    start: {
      ...
      ja: { descriptionPrompt: 'Archifyのworkflowモードを使い、次の説明を図にしてください: [関係者、主要ステップ、判断、承認、例外経路を貼り付け]。担当者ごとにレーンを分け、明確な正常系を1つ維持し、不明な担当者や未解決の分岐は推測せず明示してください。' },
    },
    signals: [['agent tool call', 16], ['tool call', 12], ['approval gate', 10], ['human in the loop', 9], ['mcp', 7], ['planner', 6], ['agent loop', 10], ['智能体工具调用', 16], ['工具调用', 12], ['审批门', 10], ['人在回路', 9], ['规划器', 6], ['智能体循环', 10], ['Agentのツール呼び出し', 16], ['ツール呼び出し', 12], ['承認ゲート', 10], ['ヒューマンインザループ', 9], ['プランナー', 6], ['Agentループ', 10]],
    en: {
    ...
    ja: {
      title: 'Agentのツール呼び出しループ', question: 'Agentはどのように計画し、許可を得て、実行し、復旧し、報告しますか？',
      summary: 'ポリシーゲート、ツール実行、例外からの復旧、証拠、最終応答をレーンで示すAgentループです。',
      useWhen: 'Agent Runtime、MCPやツールのオーケストレーション、承認、再試行、可観測性の説明に使用します。',
      avoidWhen: '静的なAgentコンポーネントだけを示す場合や、APIメッセージの正確なタイミングが主題の場合は使用しません。',
      include: ['リクエストと計画', 'ポリシーまたは承認ゲート', 'ツール実行', '例外と証拠の経路'],
      prompt: 'Archifyのworkflowモードを使い、このAgentのツール呼び出しループを説明してください。ユーザー画面、Agent Runtime、ポリシー境界、例外処理、ツール実行、可観測性をレーンに分けてください。正常系を主経路とし、承認、再試行、ブロック、証拠の経路を明示してください。',
    },
  },
  {
    id: 'delivery-workflow', type: 'workflow', proof: 'delivery-workflow',
    presentation: { preset: 'classic', motion: 'trace', views: 'optional' },
    signals: [['ci/cd', 14], ['release workflow', 14], ['deployment pipeline', 11], ['pull request', 7], ['staging', 7], ['rollback', 8], ['发布流程', 14], ['流水线', 9], ['上线', 7], ['预发', 7], ['回滚', 8], ['审批发布', 10], ['リリースワークフロー', 14], ['デプロイパイプライン', 11], ['Pull Request', 7], ['ステージング', 7], ['ロールバック', 8], ['リリース承認', 10]],
    en: {
    ...
    ja: {
      title: 'デリバリーワークフロー', question: '変更をコミットから本番環境まで安全に進めるにはどうしますか？',
      summary: 'ビルド、チェック、環境、承認、スモークテスト、ロールバック、担当レーンを含むデリバリーフローです。',
      useWhen: 'CI/CD設計、リリースレビュー、デプロイガバナンス、開発者向けデリバリーのオンボーディングに使用します。',
      avoidWhen: 'インフラの実行場所や、デプロイオブジェクトが取り得る状態が主題の場合は使用しません。',
      include: ['トリガーとビルド', 'ブロッキングチェック', '承認と環境', 'ロールバックと検証'],
      prompt: 'Archifyのworkflowモードを使い、コミットから本番環境までのデリバリープロセスを描いてください。開発者、CI、承認、環境、例外をレーンに分け、ブロッキングチェック、スモークテスト、担当者、ロールバック経路を示してください。ひと目で分かる正常系を1つ維持してください。',
    },
  },
  {
    id: 'incident-runbook', type: 'workflow', proof: 'incident-runbook',
    presentation: { preset: 'signal-flow', motion: 'trace', views: 'recommended' },
    signals: [['incident response', 15], ['runbook', 12], ['outage', 9], ['triage', 8], ['mitigation', 8], ['escalation', 7], ['事故处置', 15], ['故障', 9], ['应急预案', 12], ['排障', 9], ['缓解', 7], ['升级响应', 8], ['インシデント対応', 15], ['障害', 9], ['Runbook', 12], ['トリアージ', 8], ['影響緩和', 8], ['エスカレーション', 7]],
    en: {
    ...
    ja: {
      title: 'インシデント対応Runbook', question: '対応者はどのように検知、トリアージ、影響緩和、検証、エスカレーションを行いますか？',
      summary: 'シグナル、対応者、影響緩和、連絡、復旧の証拠を分けて示す運用ワークフローです。',
      useWhen: 'インシデント対応手順、オンコールの引き継ぎ、信頼性レビュー、机上演習に使用します。',
      avoidWhen: '対応行動ではなく、ライブ指標やインシデント後のコンポーネントトポロジーが必要な場合は使用しません。',
      include: ['検知シグナル', 'トリアージ担当者', '影響緩和とロールバック', '検証と連絡'],
      prompt: 'Archifyのworkflowモードを使い、このインシデント対応Runbookを対応者別のレーンにしてください。検知、トリアージ、影響緩和、エスカレーション、連絡、ロールバック、復旧検証を示してください。判断ゲートと行動を分け、不明な担当者を明示してください。',
    },
  },
  {
    ...
    start: {
      ...
      ja: { descriptionPrompt: 'Archifyのsequenceモードを使い、次のやり取りを描いてください: [参加者、呼び出し、戻り、フォールバック、非同期の副作用を貼り付け]。メッセージ順序を明確にし、ラベルを短く保ち、不明な動作を明示してください。リポジトリは不要です。' },
    },
    signals: [['api request', 14], ['request response', 12], ['call chain', 11], ['cache miss', 13], ['jwt', 8], ['who calls whom', 12], ['api 请求', 14], ['请求响应', 12], ['调用链', 11], ['缓存未命中', 13], ['谁调用谁', 12], ['鉴权链路', 9], ['APIリクエスト', 14], ['リクエストと応答', 12], ['呼び出しチェーン', 11], ['キャッシュミス', 13], ['誰が誰を呼び出す', 12], ['認証経路', 9]],
    en: {
    ...
    ja: {
      title: 'APIリクエストチェーン', question: '誰が誰をどの順序で呼び出し、何が返されますか？',
      summary: '認証、キャッシュのフォールバック、永続化、戻り通信、非同期トレースを時間順に示すリクエスト経路です。',
      useWhen: 'APIドキュメント、リクエストレイテンシーの調査、認証レビュー、キャッシュのフォールバック説明に使用します。',
      avoidWhen: '順序が重要でなく、安定したサービストポロジーだけが必要な場合は使用しません。',
      include: ['呼び出し元と呼び出し先', 'リクエストと戻りメッセージ', 'フォールバックまたはエラー経路', '非同期の副作用'],
      prompt: 'Archifyのsequenceモードを使い、呼び出し元から最終応答までのリクエストを示してください。認証、キャッシュヒットまたはミス、永続化へのフォールバック、戻りメッセージ、非同期のトレースまたはイベント送信を含めてください。メッセージラベルを短く保ち、順序を明確にしてください。',
    },
  },
  {
    id: 'async-roundtrip', type: 'sequence', proof: 'async-roundtrip',
    presentation: { preset: 'signal-flow', motion: 'trace', views: 'recommended' },
    signals: [['async roundtrip', 14], ['webhook', 10], ['callback', 10], ['acknowledgement', 8], ['timeout', 7], ['retry message', 8], ['异步回调', 14], ['回调', 10], ['确认消息', 8], ['超时', 7], ['消息重试', 9], ['webhook', 10], ['非同期の往復', 14], ['Webhook', 10], ['コールバック', 10], ['確認応答', 8], ['タイムアウト', 7], ['メッセージ再試行', 8]],
    en: {
    ...
    ja: {
      title: '非同期の往復処理', question: '最初のリクエストが返された後に何が起こりますか？',
      summary: 'キュー投入、確認応答、バックグラウンド処理、コールバック、再試行、タイムアウト、最終的な整合性を示すシーケンス図です。',
      useWhen: 'Webhook、ジョブ、キュー、決済コールバック、結果整合性、非同期API契約に使用します。',
      avoidWhen: '時間順序ではなく、Topicトポロジーとコンシューマーの担当範囲が主題の場合は使用しません。',
      include: ['最初の確認応答', 'キューまたはスケジューラー', 'バックグラウンド処理', 'コールバック、再試行、タイムアウト'],
      prompt: 'Archifyのsequenceモードを使い、この非同期の往復処理を説明してください。最初の確認応答、キュー投入またはスケジュール、バックグラウンド処理、コールバックまたはポーリング、再試行とタイムアウト、呼び出し元が最終的な整合性を確認できる時点を示してください。',
    },
  },
  {
    id: 'data-lineage', type: 'dataflow', proof: 'product-analytics',
    presentation: { preset: 'classic', motion: 'trace', views: 'recommended' },
    signals: [['data lineage', 15], ['etl', 12], ['warehouse', 9], ['pii', 11], ['governance', 9], ['analytics pipeline', 12], ['数据血缘', 15], ['数据管道', 11], ['数仓', 9], ['治理', 9], ['隐私数据', 10], ['用户同意', 9], ['データリネージ', 15], ['データパイプライン', 11], ['ウェアハウス', 9], ['ガバナンス', 9], ['個人情報', 10], ['ユーザー同意', 9]],
    en: {
    ...
    ja: {
      title: 'データリネージ', question: 'データはどこから来て、どのように変化し、誰が利用しますか？',
      summary: 'ソースから同意、変換、機密ストア、ウェアハウス、利用者までのガバナンスされた経路です。',
      useWhen: '分析アーキテクチャ、ETL/ELTレビュー、PII評価、ウェアハウス設計、モデル特徴量のリネージに使用します。',
      avoidWhen: 'データ資産ではなく、リクエストのタイミングや運用タスクの担当者が必要な場合は使用しません。',
      include: ['ソースとデータ資産', '変換ステージ', '分類または同意', 'ストアと利用者'],
      prompt: 'Archifyのdataflowモードを使い、このデータリネージを整理してください。すべてのデータ資産と変換に名前を付け、同意または分類の境界を示し、ストリーミングとバッチの経路を区別し、ストアと下流の利用者を特定してください。ラベルのないフローは使用しないでください。',
    },
  },
  {
    ...
    start: {
      ...
      ja: { descriptionPrompt: 'Archifyのdataflowモードを使い、次のデータの流れを整理してください: [ソース、データ資産、変換、ストア、境界、利用者を貼り付け]。すべてのフローにラベルを付け、必要に応じてストリーミングとバッチを区別し、不明な分類や担当範囲は推測せず明示してください。' },
    },
    signals: [['event stream', 15], ['kafka topology', 14], ['topic', 8], ['consumer group', 11], ['dead letter', 10], ['dlq', 10], ['事件流', 15], ['kafka 拓扑', 14], ['主题', 7], ['消费者组', 11], ['死信', 10], ['事件地铁图', 12], ['イベントストリーム', 15], ['Kafkaトポロジー', 14], ['Topic', 8], ['コンシューマーグループ', 11], ['デッドレター', 10], ['イベント路線図', 12]],
    en: {
    ...
    ja: {
      title: 'イベントストリームのトポロジー', question: 'どのイベントが、どのTopic、プロセッサー、コンシューマーグループ、失敗経路を通りますか？',
      summary: 'プロデューサー、Topic、順序付きプロセッサー、コンシューマーグループ、状態、リプレイ、DLQを示すストリーム図です。',
      useWhen: 'Kafkaやイベントプラットフォームの設計、ストリーム処理レビュー、担当範囲、リプレイ、失敗処理に使用します。',
      avoidWhen: 'Topic名、コンシューマーグループ、配信セマンティクスが不明な場合は、イベントトポロジーを推測せず汎用ワークフローを使用します。',
      include: ['プロデューサーとイベント名', 'Topicと順序', 'プロセッサーとコンシューマーグループ', '状態、リプレイ、DLQ'],
      prompt: 'Archifyのdataflowモードを使い、このイベントストリームのトポロジーを描いてください。プロデューサー、イベント、Topic、順序付きプロセッサー、コンシューマーグループ、状態ストア、リプレイ経路、DLQに名前を付けてください。担当範囲と配信セマンティクスは証拠がある場合のみ示してください。',
    },
  },
  {
    ...
    start: {
      ...
      ja: { descriptionPrompt: 'Archifyのlifecycleモードを使い、次のオブジェクトをモデル化してください: [状態、遷移イベント、待機、再試行、キャンセル、終了結果を貼り付け]。アクティブ、待機、復旧可能な失敗、終了状態を分け、終了経路を隠さないでください。リポジトリは不要です。' },
    },
    signals: [['state machine', 15], ['object lifecycle', 14], ['status transition', 11], ['terminal state', 9], ['retry state', 8], ['状态机', 15], ['生命周期', 13], ['状态流转', 11], ['终态', 9], ['等待态', 8], ['重试状态', 8], ['状態マシン', 15], ['オブジェクトのライフサイクル', 14], ['状態遷移', 11], ['終了状態', 9], ['待機状態', 8], ['再試行状態', 8]],
    en: {
    ...
    ja: {
      title: 'オブジェクトのライフサイクル', question: 'どの状態が存在し、どのイベントで遷移し、どのように終了しますか？',
      summary: 'アクティブな作業、待機、再試行、キャンセル、失敗、明示的な終了結果を含む状態モデルです。',
      useWhen: 'タスク、注文、チケット、サブスクリプション、ジョブ、Agent実行など、状態を永続化するオブジェクトに使用します。',
      avoidWhen: 'オブジェクトに永続状態がなく、時間に沿った参加者間のやり取りが主題の場合は使用しません。',
      include: ['開始状態とアクティブ状態', 'イベント名の付いた遷移', '待機状態と再試行状態', 'すべての終了結果'],
      prompt: 'Archifyのlifecycleモードを使い、このオブジェクトをモデル化してください。主要な進行、待機または中断状態、終了結果を分けてください。遷移にイベント名を付け、実際に存在する再試行、キャンセル、タイムアウト、成功、失敗を含め、終了経路を隠さないでください。',
    },
  },
  {
    id: 'deployment-lifecycle', type: 'lifecycle', proof: 'deployment-lifecycle',
    presentation: { preset: 'signal-flow', motion: 'trace', views: 'recommended' },
    signals: [['deployment lifecycle', 15], ['release state', 10], ['promotion state', 9], ['approval status', 8], ['rollback state', 10], ['部署生命周期', 15], ['发布状态', 10], ['晋级', 7], ['审批状态', 8], ['回滚状态', 10], ['デプロイのライフサイクル', 15], ['リリース状態', 10], ['昇格状態', 9], ['承認状態', 8], ['ロールバック状態', 10]],
    en: {
    ...
    ja: {
      title: 'デプロイのライフサイクル', question: 'リリースは現在どの状態にあり、次に何が起こり得ますか？',
      summary: '待機、ビルド、検証、承認、昇格、ロールバック、終了結果を含むデプロイ状態モデルです。',
      useWhen: 'リリースコントローラー、GitOpsの調整、環境への昇格、デプロイ状態APIに使用します。',
      avoidWhen: 'デプロイオブジェクトの状態ではなく、人やCIによるデリバリー行動の順序が主題の場合は使用しません。',
      include: ['待機状態と実行状態', '検証と承認', '昇格とロールバック', '成功、失敗、キャンセル'],
      prompt: 'Archifyのlifecycleモードを使い、デプロイオブジェクトをモデル化してください。待機、ビルド、検証、承認待ち、昇格、ロールバック、すべての終了結果を示してください。各遷移を許可するイベントとガード条件にラベルを付けてください。',
    },
  },
];
...
export function detectGuideLanguage(value = '') {
  ...
  if (/[这为个与归处过还从数据线络图仓区实门问时发经应展示请求]/u.test(text)) return 'zh';
  if (/[\u3400-\u9fff]/u.test(text)) return 'ja';
  return 'en';
}

export function startPromptsFor(recipe, lang = 'ja') {
  const language = ['ja', 'zh', 'en'].includes(lang) ? lang : 'ja';
  const copy = recipe[language];
  const descriptionPrompt = recipe.start?.[language]?.descriptionPrompt;
  if (!descriptionPrompt) {
    throw new Error(`シナリオレシピ${JSON.stringify(recipe.id)}には${language}の開始プロンプトが定義されていません。`);
  }
  const repositoryPrompt = recipe.type === 'architecture'
    ...
    : language === 'zh'
      ...
      : language === 'en'
        ...
        : `まず、このリポジトリ内の関連する証拠を確認してください。その後、${copy.prompt}コードで裏付けられない動作を推測しないでください。`;
...
function localized(recipe, lang) {
  const copy = recipe[['ja', 'zh', 'en'].includes(lang) ? lang : 'ja'];
  return {
    id: recipe.id,
    type: recipe.type,
    proof: recipe.proof,
...
export function recommendScenario(query, options = {}) {
  const lang = ['ja', 'zh', 'en'].includes(options.lang) ? options.lang : detectGuideLanguage(query);
...
export function formatScenarioList(lang = 'ja') {
  ...
  const isZh = lang === 'zh';
  const heading = isJa
    ? `Archifyシナリオレシピ（${SCENARIO_RECIPES.length}件）`
    : isZh
    ? `Archify 场景配方（${SCENARIO_RECIPES.length}）`
    : `Archify scenario recipes (${SCENARIO_RECIPES.length})`;
  const intro = isJa
    ? '図の種類より先に、答えたい問いを選んでください。実行例: archify guide "シナリオ"'
    : isZh
...
export function formatScenarioRecommendation(result) {
  const isJa = result.lang === 'ja';
  const isZh = result.lang === 'zh';
  const recipe = result.recommendation;
  const labels = isJa ? {
    heading: '推奨', question: '答える問い', use: '適した用途', avoid: '適さない用途', include: '必須要素', presentation: '表示の提案', prompt: 'そのまま使えるプロンプト', alternatives: 'その他の候補', confidence: '確信度', views: 'ビュー',
  } : isZh ? {
  ...
  const lines = [
    ...
    `${labels.include}: ${recipe.include.join(isJa || isZh ? '、' : '; ')}`,
    `${labels.presentation}: ${recipe.presentation.preset} · ${recipe.presentation.motion} · ${isJa ? labels.views : 'views'} ${recipe.presentation.views}`,
    '',
    `${labels.prompt}:`,
    recipe.prompt,
  ];
