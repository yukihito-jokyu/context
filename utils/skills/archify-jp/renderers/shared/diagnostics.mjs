...
function normalizedDiagnostic(diagnostic) {
  const message = String(diagnostic?.message || 'Archifyはこの失敗を分類できませんでした。').trim();
  return {
...
function fallbackDiagnostic(error) {
  ...
  if (error instanceof SyntaxError) {
    return normalizedDiagnostic({
      ...
      severity: 'error',
      message: `入力JSONを解析できませんでした: ${error.message}`,
      subject: { input },
      evidence: { reason: error.message },
      supportedFixes: ['JSON構文を修正して検証を再実行する'],
    });
  }
  if (error?.code === 'ENOENT' || error?.code === 'EACCES' || error?.code === 'EISDIR') {
    return normalizedDiagnostic({
      code: 'input/read',
      severity: 'error',
      message: `入力を読み込めませんでした: ${error.message}`,
      subject: { input },
      evidence: { systemCode: error.code, reason: error.message },
      supportedFixes: ['読み取り可能なJSON入力ファイルを1つ指定する'],
    });
  }
  return normalizedDiagnostic({
    code: 'internal/unclassified',
    severity: 'error',
    message: error?.message || '診断情報がないままレンダラーが失敗しました。',
    subject: { input },
...
function rendererFailure(error) {
  ...
  return {
    ...
    ok: false,
    source: 'renderer',
    error: error?.message || '診断情報がないままレンダラーが失敗しました。',
    diagnostics,
  };
}
