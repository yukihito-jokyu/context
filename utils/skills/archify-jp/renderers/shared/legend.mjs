...
export function measureLegend(entries, {
  ...
  if (tooWide) {
    ...
    const message = `[legend/label-too-wide] ${diagramType}の図例ラベル "${tooWide.kind}" には${tooWide.width}px必要ですが、利用できる幅は${width}pxだけです。`;
    throwDiagnosticError(message, [{
      ...
      supportedFixes: ['図例ラベルを短くするか、より幅広いviewBoxを使用する'],
    }]);
  }
  ...
  if (legendTopY < minTitleY) {
    if (unfit === 'hide') return null;
    const message = `[legend/vertical-overflow] ${diagramType}の図例には${footprint.rowCount}行必要で、y=${minTitleY}から利用できる図例領域より上のy=${legendTopY}から始まってしまいます。`;
    throwDiagnosticError(message, [{
      ...
      supportedFixes: ['図例ラベルを短くする、不要な項目を非表示にする、またはより幅広いviewBoxを使用する'],
    }]);
  }
  ...
  if (collision) {
    if (unfit === 'hide') return null;
    const message = `[legend/content-overlap] ${diagramType}の図例項目 "${collision.kind}" が作成済みの関係ジオメトリと重なっています。`;
    throwDiagnosticError(message, [{
      ...
      supportedFixes: ['図例項目を短くするか非表示にする、より幅広いviewBoxを使用する、または作成済みの関係ルート／ラベルを図例領域の外へ移動する'],
    }]);
  }
