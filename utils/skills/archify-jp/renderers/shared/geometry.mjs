...
export function cleanEndpointSideProblems({
  ...
  routeHint = '最初／最後のvia区間をfromSide／toSideに揃える、sideを変更する、または明示的な経路指定を削除してautoに垂直な進入を選択させる',
}) {
  const problems = [];
  for (const [relationIndex, relation] of asArray(relations).entries()) {
    ...
    for (const issue of checks) {
      ...
      const sideField = issue.sideOrigin === 'inferred' ? `推定${authoredField}` : authoredField;
      const segmentRole = issue.endpoint === 'source' ? '最初' : '最後';
      const from = issue.start.map((value) => Math.round(value * 10) / 10).join(', ');
      const to = issue.end.map((value) => Math.round(value * 10) / 10).join(', ');
      const message = `[clean-flow/endpoint-side-direction] ${diagramType} ${relationCollection}[${relationIndex}]${relationId}「${relation.from}」->「${relation.to}」の${segmentRole}の区間${issue.segmentIndex} [${from}] -> [${to}]が${sideField}「${issue.side}」に従っていません。${issue.expectedAxis}軸の${issue.expectedDirection}方向でなければなりません。${routeHint}。`;
      recordDiagnostic({
...
export function cleanFlowProblems({
  ...
  obstacleKind,
  clearance = 2,
  routeHint = 'fromSide／toSideを調整する、route／viaまたはchannel座標を設定する、あるいは障害物を移動する'
}) {
  ...
  for (const [relationIndex, relation] of asArray(relations).entries()) {
    ...
    for (const obstacle of obstacleList) {
      ...
      const relationId = relation.id ? ` id「${relation.id}」` : '';
      const message = `[clean-flow/edge-through-node] ${diagramType} ${relationCollection}[${relationIndex}]${relationId}「${relation.from}」->「${relation.to}」が、区間${hitSegment} [${from}] -> [${to}]で、この関係と無関係な${obstacleKind}「${obstacle.id}」を横切っています（クリアランス${clearance}px）。${routeHint}。`;
      recordDiagnostic({
...
export function cleanCrossingProblems({
  ...
  routeHint = '関係が別々の経路を使用するよう、route／viaまたはchannel座標を調整する'
}) {
  ...
  for (let leftIndex = 0; leftIndex < routed.length; leftIndex += 1) {
    ...
    for (let rightIndex = leftIndex + 1; rightIndex < routed.length; rightIndex += 1) {
      ...
      const message = `[composition/proper-crossing] showcase ${diagramType} ${describe(left)}が[${point}]で${describe(right)}と交差しています（区間${hit.leftSegment}と${hit.rightSegment}）。${routeHint}。`;
      recordDiagnostic({
...
export function cleanAmbiguousCorridorProblems({
  ...
  return collectAmbiguousCorridors({ routedRelations, minOverlapPx }).map((hit) => {
    ...
    const message = `[composition/ambiguous-corridor] showcase ${diagramType} ${describe(hit.left)}が[${from}] -> [${to}]で${describe(hit.right)}と${length}pxの経路を共有しています（区間${hit.leftSegment}と${hit.rightSegment}、最小${minOverlapPx}px）。${routeHint}。`;
    recordDiagnostic({
...
export function cleanBorderRunProblems({
  ...
  profile,
  profileIsAuthoritative = false,
  routeHint = '関係が空いている開口部を通り枠と垂直に交差するよう、route／viaまたはchannel座標を調整する'
}) {
  ...
  return collectBorderRuns({ routedRelations, frames }).map((hit) => {
    ...
    const from = hit.overlapStart.map((value) => Math.round(value * 10) / 10).join(', ');
    const to = hit.overlapEnd.map((value) => Math.round(value * 10) / 10).join(', ');
    const message = `[composition/container-border-run] ${diagramType} ${relationCollection}[${hit.relationIndex}]${relationId}「${relation.from}」->「${relation.to}」の区間${hit.segmentIndex} [${from}] -> [${to}]が、${frameKind}「${frameIdentity}」の${hit.side}境界に${length}px沿っています。${routeHint}。`;
    recordDiagnostic({
...
export function cleanRouteRhythmProblems({
  ...
  profile,
  profileIsAuthoritative = false,
  routeHint = '窮屈な折れをなくすようchannel／via点を移動するか、経路により広い通路を与える',
  interiorSegmentPx = 16,
  microSegmentPx = 8,
}) {
  if (qualityProfileForGate(profile, profileIsAuthoritative) !== 'showcase') return [];
  ...
  return collectRouteRhythmIssues({ routedRelations, interiorSegmentPx, microSegmentPx }).map((hit) => {
    ...
    const rule = hit.code === 'composition/micro-segment'
      ? `${microSegmentPx}pxの微小区間下限を下回っています`
      : `${interiorSegmentPx}pxの内部区間下限を下回っています`;
    const message = `[${hit.code}] showcase ${diagramType} ${relationCollection}[${hit.relationIndex}]${relationId}「${relation.from}」->「${relation.to}」の${hit.position}区間${hit.segmentIndex} [${from}] -> [${to}]は${length}pxで、${rule}。${routeHint}。`;
    recordDiagnostic({
      code: hit.code,
      severity: 'error',
      message,
...
export function cleanLabelRouteClearanceProblems({
  ...
  profileIsAuthoritative = false,
  threshold = 4,
  routeHint = 'labelAt、labelDx、labelDy、labelSegmentを調整する。それ以外では、もう一方の関係のroute／via／channelを調整する',
}) {
  if (qualityProfileForGate(profile, profileIsAuthoritative) !== 'showcase') return [];
  const routedRelations = collectEligibleRoutedRelations({ relations, endpointIds, pathFor });
  return collectLabelRouteClearance({ labels, routedRelations, threshold }).map((hit) => {
    ...
    const from = hit.start.map((value) => Math.round(value * 10) / 10).join(', ');
    const to = hit.end.map((value) => Math.round(value * 10) / 10).join(', ');
    const message = `[composition/label-route-clearance] showcase ${diagramType} ${describe(hit.labelRelation, hit.labelRelationIndex)}上のlabel「${hit.label?.label || hit.labelRelation?.label || ''}」は、${describe(hit.otherRelation, hit.otherRelationIndex)}の区間${hit.segmentIndex} [${from}] -> [${to}]から${clearance}pxです（labelの矩形${formatRect(hit.rect)}、最小${threshold}px）。${routeHint}。`;
    recordDiagnostic({
...
export function suggestLabelObstacleFix(labelRect, lx, ly, obstacle, obstacleKind = 'コンポーネント') {
  ...
  return [
    `  labelの矩形: ${formatRect(labelRect)}`,
    `  ${obstacleKind}「${obstacle.id}」の矩形: ${formatRect(obstacle)}`,
    `  修正案: labelAt [${lxR}, ${belowY}]またはlabelDy ${formatDelta(belowY - lyR)}（下）、あるいはlabelAt [${lxR}, ${aboveY}]またはlabelDy ${formatDelta(aboveY - lyR)}（上）`,
  ].join('\n');
}
...
export function suggestLabelPairFix(a, b) {
  return [
    ...
    '  修正案: labelDx／labelDy／labelSegmentを調整するか、一方の関係を別の経路へ通す',
  ].join('\n');
}
...
export function suggestComponentSeparation(a, b, minGap = 8) {
  ...
  return [
    ...
    `  修正案: 「${b.id}」のposを[${rightX}, ${Math.round(b.y)}]（「${a.id}」の右）または[${Math.round(b.x)}, ${belowY}]（下）へ移動する`,
  ].join('\n');
}
