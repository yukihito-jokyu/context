...
export function validateRelationshipIds(diagramType, diagram) {
  ...
  relationships.forEach((relationship, index) => {
    ...
    if (seen.has(relationship.id)) {
      problems.push(`/${collection}/${index}/idで関係ID ${JSON.stringify(relationship.id)} が重複しています`);
    }
    seen.add(relationship.id);
  });

  if (problems.length) {
    throwDiagnosticProblems('関係識別情報の検証に失敗しました', problems, {
...
export function validateGuidedViews(diagramType, diagram) {
  ...
  views.forEach((view, index) => {
    if (seen.has(view.id)) problems.push(`/meta/views/${index}/idでビューID ${JSON.stringify(view.id)} が重複しています`);
    seen.add(view.id);
    const seenFocus = new Set();
    (view.focus || []).forEach((id, focusIndex) => {
      if (seenFocus.has(id)) {
        problems.push(`/meta/views/${index}/focus/${focusIndex}で意味ID ${JSON.stringify(id)} が重複しています`);
      }
      seenFocus.add(id);
      if (!semanticIds.has(id)) {
        problems.push(`/meta/views/${index}/focus/${focusIndex}が不明な意味ID ${JSON.stringify(id)} を参照しています`);
      }
    });
  });

  if (problems.length) {
    throwDiagnosticProblems('ガイド付きビューの検証に失敗しました', problems, {
