...
function validateUrlShape(url, allowPrivate = process.env.ARCHIFY_BRAND_ALLOW_PRIVATE === '1') {
  if (!['https:', 'http:'].includes(url.protocol)) throw new Error('ブランドリンクはHTTP(S)だけに対応しています');
  if (url.username || url.password) throw new Error('ブランドリンクに認証情報を含めることはできません');
  const expectedPort = url.protocol === 'https:' ? '443' : '80';
  if (!allowPrivate && url.port && url.port !== expectedPort) {
    throw new Error('ブランドリンクは標準Webポートを使用する必要があります');
  }
  ...
  if (!allowPrivate && (host === 'localhost' || host.endsWith('.localhost') || host.endsWith('.local'))) {
    throw new Error('プライベートなブランドリンクは取得しません');
  }
  return host;
}
...
function beforeDeadline(promise, deadline) {
  ...
  if (remaining <= 0) return Promise.reject(new Error('ブランド取得がタイムアウトしました'));
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('ブランド取得がタイムアウトしました')), remaining);
    timer.unref?.();
    promise.then(
...
async function checkedFetch(input, accept, deadline) {
  ...
  for (let redirects = 0; redirects <= 3; redirects += 1) {
    if (Date.now() >= deadline) throw new Error('ブランド取得がタイムアウトしました');
    ...
    if ([301, 302, 303, 307, 308].includes(response.status)) {
      ...
      if (!location || redirects === 3) throw new Error('ブランドリンクのリダイレクト回数が多すぎます');
      current = new URL(location, current);
      continue;
    }
    if (!response.ok) {
      response.body.resume();
      throw new Error(`ブランドリンクがHTTP ${response.status}を返しました`);
    }
    return { response, finalUrl: current };
  }
  throw new Error('ブランドリンクのリダイレクト回数が多すぎます');
}
...
async function readLimited(response, maximum) {
  ...
  if (Number.isFinite(declared) && declared > maximum) {
    ...
    throw new Error('ブランドアセットが大きすぎます');
  }
  ...
  if (!response.body?.getReader) {
    ...
    if (buffer.length > maximum) throw new Error('ブランドアセットが大きすぎます');
    return buffer;
  }
...
async function imageData(response) {
  ...
  if (!allowed.has(contentType)) {
    response.body?.destroy?.();
    throw new Error(`未対応のブランド画像形式です: ${contentType || '不明'}`);
  }
  ...
  if (!signatureMatches) throw new Error(`ブランドアセットのバイト列が${contentType}と一致しません`);
  return {
...
async function captureRemoteBrand(value, deadline = Date.now() + captureTimeoutMilliseconds()) {
  ...
  try {
    ...
    if (!pageType.includes('text/html') && !pageType.includes('application/xhtml+xml')) {
      ...
      return fallback('リンク先ページがHTMLではありません');
    }
    ...
    const usefulError = iconErrors.find((error) => /未対応のブランド画像形式/.test(error?.message))
      || iconErrors.at(-1);
    return fallback(usefulError?.message || '使用可能なサイトアイコンが見つかりませんでした');
  } catch (error) {
...
export async function captureBrandReference(value) {
  const url = asUrl(value);
  if (!url) throw new Error('ブランド取得にはHTTP(S) URLが1つ必要です');
  ...
  if (resolved.status !== 'captured' || !resolved.sha256) {
    throw new Error(`ブランド取得に失敗しました: ${resolved.reason || '使用可能なサイトアイコンが見つかりませんでした'}`);
  }
  return {
  ...
  await mapConcurrent(nodes, MAX_CAPTURE_CONCURRENCY, async (node, index) => {
    ...
    if (typeof node.brand === 'object') {
      ...
      if (!resolved || resolved.status !== 'captured') {
        unknown.push(`/${collection}/${index}/brandで固定済み取得を再現できませんでした: ${resolved?.reason || '無効なURL'}`);
        return;
      }
      if (resolved.sha256 !== node.brand.sha256) {
        unknown.push(`/${collection}/${index}/brandのダイジェストが変更されました: 期待値${node.brand.sha256}、受信値${resolved.sha256}`);
        return;
      }
    ...
    if (url) {
      unknown.push(`/${collection}/${index}/brand ${JSON.stringify(node.brand)}は固定されていないURLです。先に\`archify brands capture ${url.href} --json\`で取得してください`);
      return;
    }
    unknown.push(`/${collection}/${index}/brand ${JSON.stringify(node.brand)}は組み込みブランドではありません。最も近いID: ${suggestions(node.brand).join(', ')}`);
  });
  if (unknown.length) {
    throwDiagnosticError(`ブランドマークの検証に失敗しました:\n- ${unknown.join('\n- ')}`, unknown.map((message) => ({
      code: message.includes('固定されていないURL') ? 'brand/unpinned-url'
        : (message.includes('ダイジェストが変更されました') ? 'brand/digest-mismatch'
          : (message.includes('固定済み取得を再現できませんでした') ? 'brand/capture-unavailable' : 'brand/unknown')),
      severity: 'error',
      message,
      subject: { diagramType, collection },
      evidence: {},
      supportedFixes: message.includes('固定されていないURL')
        ? ['`archify brands capture <url> --json`を実行し、返されたダイジェスト固定済みbrandオブジェクトを作成する']
        : ['`archify brands`からIDを選択する', '未知の公式サイトには`archify brands capture <url> --json`を実行する'],
    })));
  }
}
...
export function brandTopRailProblem(node, width, minimumFontSize, subject = 'ノード') {
  ...
  return `${subject}「${node.id}」のブランド上部レールでラベルに使える幅は${Math.max(0, available)}pxですが、`
    + `「${node.label}」は可読最小値${minimumFontSize}pxで約${Math.ceil(required)}pxを必要とします。ノードを広げるか、ラベルを短くしてください。`;
}
