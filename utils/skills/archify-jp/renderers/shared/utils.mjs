...
const SUBTITLE_SLOT_RE = /^([ \t]*)<p class="subtitle">\[サブタイトルの説明\]<\/p>[ \t]*(\r?\n)?/m;
...
const TEMPLATE_PLACEHOLDERS = [
  '<html lang="ja" data-theme="dark" data-preset="[VISUAL PRESET]">',
  '<title>[PROJECT NAME] アーキテクチャ図</title>',
  '<h1>[PROJECT NAME] アーキテクチャ</h1>',
...
export function applyTemplate(template, {
  ...
  if (!SVG_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: テンプレートに ARCHIFY:SVG_SLOT センチネルがありません');
  }
  if (!CARDS_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: テンプレートに ARCHIFY:CARDS_SLOT センチネルがありません');
  }
  if (!SUBTITLE_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: テンプレートにサブタイトルのプレースホルダーがありません');
  }
  for (const ph of TEMPLATE_PLACEHOLDERS) {
    if (!template.includes(ph)) {
      throw new Error(`applyTemplate: テンプレートにプレースホルダー ${JSON.stringify(ph)} がありません`);
    }
  }
  ...
  if (sourceEvidence && !template.includes(SOURCE_EVIDENCE_PLACEHOLDER)) {
    throw new Error(`applyTemplate: リポジトリの証拠にはプレースホルダー ${JSON.stringify(SOURCE_EVIDENCE_PLACEHOLDER)} が必要です`);
  }
