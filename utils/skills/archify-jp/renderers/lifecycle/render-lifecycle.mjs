...
function validateLifecycle() {
  const problems = [];
  if (states.size !== asArray(lifecycle.states).length) problems.push('状態の id は一意である必要があります。');
  ...
  if (lifecycleAreaBottom() + 4 < 448) {
    problems.push(`viewBox の高さ ${viewBox[1]} は固定バンドレイアウトには不足しています。meta.viewBox[1] を 566 以上に設定してください。`);
  }

  const laneIds = new Set(asArray(lifecycle.lanes).map((lane) => lane.id));
  if (laneIds.size !== asArray(lifecycle.lanes).length) problems.push('レーンの id は一意である必要があります。');
  if (!laneIds.has('main')) {
    problems.push('ライフサイクル図には id が "main" のレーン（フェーズレール）が必要です。レーン id "main" と "terminal" は予約済みです。"main" は上部のフェーズバンド、"terminal" は下部の結果バンドに対応し、その他のレーンは中央のイベントバンドを共有します。');
  }

  for (const state of states.values()) {
    if (!laneIds.has(state.lane)) {
      problems.push(`状態 "${state.id}" が不明なレーン "${state.lane}" を使用しています。`);
      continue;
    }
    ...
    if (!Number.isInteger(state.col) || state.col < 0 || state.col >= maxCol) {
      problems.push(`状態 "${state.id}" の列 ${state.col} は無効です。${band} バンドで使用できる整数列は 0..${maxCol - 1} です。`);
      continue;
    }
    if (!isFinitePoint(state.x, state.y, state.cx, state.cy)) {
      problems.push(`状態 "${state.id}" の座標が有限値ではありません。col、width、height、yOffset が数値であることを確認してください。`);
      continue;
    }
    if (state.x < 32 || state.x + state.width > viewBox[0] - 32) {
      problems.push(`状態 "${state.id}" が図の水平方向の範囲を超えています。state.width を減らすか meta.viewBox[0] を増やしてください。`);
    }
    if (state.y < 64 || state.y + state.height > lifecycleAreaBottom()) {
      problems.push(`状態 "${state.id}" がライフサイクルの垂直領域を超えています。y を 64 から ${lifecycleAreaBottom()} の間に収めてください（yOffset を調整するか meta.viewBox[1] を増やしてください）。`);
    }
    const estLabelW = textUnits(state.label) * 6.2;
    if (estLabelW > state.width + 6) {
      problems.push(`ラベル "${state.label}"（約 ${Math.round(estLabelW)}px）が状態 "${state.id}"（${state.width}px）より広くなっています。ラベルを短くするか state.width を増やしてください。`);
    }
    const brandRailProblem = brandTopRailProblem(state, state.width, 8, '状態');
    if (brandRailProblem) problems.push(brandRailProblem);
    // sublabel and tag render as single unwrapped <text> elements; shrink-to-fit
    // handles the ordinary case, this rejects what it cannot rescue.
    const availableTextW = availableNodeTextWidth(state.width);
    for (const [field, value, minimum] of [
      ['サブラベル', state.sublabel, stateTextFit.sublabelMinimum],
      ['タグ', state.tag, stateTextFit.tagMinimum],
    ]) {
      if (!value) continue;
      const minimumW = minimumNodeTextWidth(value, minimum);
      if (minimumW > availableTextW) {
        problems.push(`${field} "${value}" は可読性を保つ最小値 ${minimum}px で約 ${Math.ceil(minimumW)}px 必要ですが、状態 "${state.id}" には ${availableTextW}px しかありません。${field.toLowerCase()} を短くするか state.width を増やしてください。`);
      }
    }
  }
  ...
  for (let i = 0; i < allStates.length; i += 1) {
    for (let j = i + 1; j < allStates.length; j += 1) {
      if (rectsOverlap(allStates[i], allStates[j], 10)) {
        problems.push(`状態 "${allStates[i].id}" と "${allStates[j].id}" の間隔が 10px 未満です。一方を別の col へ移動するか yOffset で離してください（"main"/"terminal" 以外のレーンは1つのバンドを共有します）。`);
      }
    }
  }

  for (const transition of asArray(lifecycle.transitions)) {
    if (!states.has(transition.from)) problems.push(`遷移 "${transition.label || transition.from}" が不明な遷移元 "${transition.from}" を参照しています。`);
    if (!states.has(transition.to)) problems.push(`遷移 "${transition.label || transition.to}" が不明な遷移先 "${transition.to}" を参照しています。`);
    if (states.has(transition.from) && states.has(transition.to)) {
      const routed = pathFor(transition);
      const [start, end] = [routed.points[0], routed.points[routed.points.length - 1]];
      const distance = Math.hypot(end[0] - start[0], end[1] - start[1]);
      if (distance < 32) problems.push(`遷移 "${transition.label || `${transition.from}->${transition.to}`}" が短すぎます（${Math.round(distance)}px、最小 32px）。チャネルを経由させるか、ラベルを削除してください。`);
    }
  }
  ...
  problems.push(...cleanEndpointSideProblems({
    ...
    routeHint: '自動ルーティングを維持するか、最初と最後のセグメントが状態の境界を垂直に横切る fromSide/toSide と via 点を選ぶ',
  }));
  problems.push(...cleanFlowProblems({
    ...
    relationCollection: 'transitions',
    obstacleKind: 'state',
    routeHint: 'fromSide/toSide を調整するか、route/via または channelX/channelY を設定するか、col/yOffset で状態を移動する'
  }));
  problems.push(...cleanCrossingProblems({
    ...
    routeHint: '遷移が別々のライフサイクル経路帯を使用するよう route/via または channelX/channelY を調整する'
  }));
  problems.push(...cleanAmbiguousCorridorProblems({
    ...
    relationCollection: 'transitions',
    profile: lifecycle.meta?.quality_profile,
    routeHint: '無関係な遷移が視覚的に合流しないよう route/via または channelX/channelY を調整する'
  }));
  ...
  problems.push(...cleanRouteRhythmProblems({
    ...
    relationCollection: 'transitions',
    profile: lifecycle.meta?.quality_profile,
    routeHint: 'ライフサイクルの各曲がり角に読み取れる助走区間ができるよう route/via またはチャネル座標を移動する'
  }));
  ...
  for (const rect of labelRects) {
    for (const state of states.values()) {
      if (rectsOverlap(rect, state, -2)) {
        problems.push(`ラベル "${rect.label}" が状態 "${state.id}" と重なっています。labelDx/labelDy/labelSegment を調整するか labelAt を設定してください。\n${suggestLabelObstacleFix(rect, rect.lx, rect.ly, state, 'state')}`);
      }
    }
  }
  ...
  if (problems.length) {
    throwDiagnosticProblems('ライフサイクルのレイアウト検証に失敗しました', problems, {
...
function bandTitles() {
  ...
  return [
    mainLane?.label || 'ライフサイクルのフェーズ',
    eventLanes.length ? eventLanes.map((lane) => lane.label).join(' + ') : '中断 + 復旧',
    terminalLane?.label || '結果'
  ];
}
