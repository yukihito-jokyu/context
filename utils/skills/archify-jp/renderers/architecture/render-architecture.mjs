...
function resolveBoundaryTitles() {
  ...
  return {
    ...
    readabilityProblem: `[composition/desktop-readability] 境界タイトルのレイアウトが ${maximumIterations} 回の反復後も最終的な幅 ${finalViewBoxWidth}px の viewBox に収束しませんでした。境界ラベルを短くするか、より幅広い viewBox を指定するか、内包されるコンポーネントを左端に近づけてください。`,
  };
}
...
function validateArchitecture() {
  ...
  if (components.size !== asArray(arch.components).length) problems.push('コンポーネントの id は一意である必要があります。');
  if (grid) {
    validateGridPlacement(arch, grid, problems);
  } else {
    for (const c of asArray(arch.components)) {
      if (!Array.isArray(c.pos) || c.pos.length !== 2) {
        problems.push(`layout.mode を省略して自由配置する場合、コンポーネント "${c.id}" に pos [x, y] を指定する必要があります。`);
      }
    }
  }
  ...
  for (const c of components.values()) {
    if (!isFinitePoint(c.x, c.y, c.width, c.height)) {
      problems.push(`コンポーネント "${c.id}" の pos/size が有限値ではありません。pos と size は [number, number] である必要があります。`);
      continue;
    }
    if (c.width <= 0 || c.height <= 0) {
      problems.push(`コンポーネント "${c.id}" のサイズ ${c.width}x${c.height} は無効です。width と height は0より大きい必要があります。`);
      continue;
    }
    if (c.x < 0 || c.y < 0 || c.x + c.width > viewBox[0] || c.y + c.height > viewBox[1]) {
      problems.push(`コンポーネント "${c.id}" が viewBox ${viewBox[0]}x${viewBox[1]} の外にあります。pos/size を調整するか、より大きな meta.viewBox を設定してください。`);
    }
    const estLabelW = textUnits(c.label) * 6.6;
    if (estLabelW > c.width + 8) {
      problems.push(`ラベル "${c.label}"（約 ${Math.round(estLabelW)}px）がコンポーネント "${c.id}"（${c.width}px）より広くなっています。ラベルを短くするか、size の幅を広げてください。`);
    }
    const brandRailProblem = brandTopRailProblem(c, c.width, 8, 'コンポーネント');
    if (brandRailProblem) problems.push(brandRailProblem);
    // sublabel and tag render as single unwrapped <text> elements; shrink-to-fit
    // handles the ordinary case, this rejects what it cannot rescue.
    const availableTextW = availableNodeTextWidth(c.width);
    for (const [field, value, minimum] of [
      ['サブラベル', c.sublabel, componentTextFit.sublabelMinimum],
      ['タグ', c.tag, componentTextFit.tagMinimum],
    ]) {
      if (!value) continue;
      const minimumW = minimumNodeTextWidth(value, minimum);
      if (minimumW > availableTextW) {
        problems.push(`${field} "${value}" は可読性を保つ最小値 ${minimum}px で約 ${Math.ceil(minimumW)}px 必要ですが、コンポーネント "${c.id}" には ${availableTextW}px しかありません。${field.toLowerCase()} を短くするか、size の幅を広げてください。`);
      }
    }
  }
  ...
  for (let i = 0; i < list.length; i += 1) {
    for (let j = i + 1; j < list.length; j += 1) {
      if (rectsOverlap(list[i], list[j], 8)) {
        problems.push(`コンポーネント "${list[i].id}" と "${list[j].id}" の間隔が 8px 未満です。一方を移動するか、サイズを縮小してください。\n${suggestComponentSeparation(list[i], list[j], 8)}`);
      }
    }
  }
  ...
  for (const boundary of asArray(arch.boundaries)) {
    for (const id of asArray(boundary.wraps)) {
      if (!components.has(id)) problems.push(`境界 "${boundary.label}" が不明なコンポーネント "${id}" を内包しています。`);
    }
  }
  ...
  for (const boundary of boundaries) {
    ...
    if (boundary.title.minimumWidth > boundary.title.availableWidth) {
      problems.push(
        `境界ラベル "${boundary.label}" をデスクトップで読める最小フォントサイズ `
        + `${Number(boundary.title.minimumFontSize.toFixed(2))}px で収めるには約 ${Math.ceil(boundary.title.minimumWidth)}px 必要ですが、フレームには ${Math.floor(boundary.title.availableWidth)}px しかありません。`
        + '境界ラベルを短くするか、pad を増やすか、内包するコンポーネントのレイアウト幅を広げてください。',
      );
    }
    if (!rectContains(boundary, boundary.title)) {
      problems.push(
        `境界ラベル "${boundary.label}" が最終フレームの外にはみ出しています。ラベルを短くするか、境界の pad を増やしてください。`,
      );
    }
    if (!rectContains(viewBoxRect, boundary.title)) {
      problems.push(
        `境界ラベル "${boundary.label}" が viewBox の外にはみ出しています。内包するコンポーネントをキャンバス端から離すか、ラベルを短くするか、viewBox を広げてください。`,
      );
    }
    for (const component of components.values()) {
      if (!rectsOverlap(boundary.title, component)) continue;
      problems.push(
        `境界ラベル "${boundary.label}" がコンポーネント "${component.id}" と重なっています。コンポーネントを移動するか、境界タイトルの領域を広げるか、ラベルを短くしてください。`,
      );
    }
  }
  for (let leftIndex = 0; leftIndex < boundaries.length; leftIndex += 1) {
    ...
    for (let rightIndex = leftIndex + 1; rightIndex < boundaries.length; rightIndex += 1) {
      ...
      if (enforcesBoundaryTitleComposition && rectsOverlap(left.title, right.title)) {
        problems.push(
          `境界ラベル "${left.label}" と "${right.label}" が重なっています。ラベルを短くするか、境界タイトルの領域を広げてください。`,
        );
      }
      ...
      if (shared.length && !leftNested && !rightNested) {
        ...
        problems.push(
          `境界 "${left.label}" と境界 "${right.label}" は、所属要素が部分的に重なるため交差しています `
          + `（共通: ${shared.map((id) => `"${id}"`).join(', ')}、`
          + `"${left.label}" のみ: ${leftOnly.map((id) => `"${id}"`).join(', ')}、`
          + `"${right.label}" のみ: ${rightOnly.map((id) => `"${id}"`).join(', ')}）。`
          + '外側の要素を除いて一方の境界を完全に入れ子にするか、境界を分割してください。',
        );
        continue;
      }
      ...
      if (!leftContainsRight && !rightContainsLeft) {
        problems.push(
          `境界 "${left.label}" と境界 "${right.label}" の最終フレームが部分的に重なっています。`
          + 'フレームが分離するか一方が他方を完全に内包するよう、wraps、pad、またはコンポーネントの位置を調整してください。',
        );
        continue;
      }
      ...
      if (!shared.length) {
        problems.push(
          `境界 "${left.label}" と境界 "${right.label}" は所属要素が別々であるにもかかわらず、最終フレームが重なっています。`
          + 'フレームが分離するよう pad またはコンポーネントの位置を調整するか、意図した入れ子関係を wraps で表してください。',
        );
        continue;
      }
      ...
      if (!containmentMatchesMembership) {
        problems.push(
          `境界 "${left.label}" と境界 "${right.label}" の最終フレームの包含関係が wraps の所属関係と矛盾しています。`
          + '内側の境界の pad を減らすか、コンポーネントを移動するか、形状と入れ子関係が一致するよう wraps を修正してください。',
        );
      }
    }
  }
  for (const b of boundaries) {
    if (b.x < 0 || b.y < 0 || b.x + b.width > viewBox[0] || b.y + b.height > viewBox[1]) {
      problems.push(`境界 "${b.label}" が viewBox の外にはみ出しています。内包する要素がキャンバス端に近すぎるため、余白を追加するか meta.viewBox を広げてください。`);
    }
  }

  for (const conn of asArray(arch.connections)) {
    if (!components.has(conn.from)) problems.push(`接続 "${conn.label || conn.from}" が不明な接続元 "${conn.from}" を参照しています。`);
    if (!components.has(conn.to)) problems.push(`接続 "${conn.label || conn.to}" が不明な接続先 "${conn.to}" を参照しています。`);
    if (components.has(conn.from) && components.has(conn.to)) {
      const routed = pathFor(conn);
      const [start, end] = [routed.points[0], routed.points[routed.points.length - 1]];
      const distance = Math.hypot(end[0] - start[0], end[1] - start[1]);
      if (distance < 24) problems.push(`接続 "${conn.label || `${conn.from}->${conn.to}`}" が短すぎます（${Math.round(distance)}px、最小 24px）。コンポーネント同士をさらに離してください。`);
    }
  }
  ...
  problems.push(...cleanEndpointSideProblems({
    ...
    routeHint: 'レンダラーが辺を考慮した橋渡しを使えるよう自動ルーティングを維持するか、垂直に交差する via セグメントと正確な fromSide/toSide を設定する',
  }));
  problems.push(...cleanFlowProblems({
    ...
    routeHint: 'fromSide/toSide を調整するか、route/via を設定するか、コンポーネントを移動する'
  }));
  problems.push(...cleanCrossingProblems({
    ...
    routeHint: '接続が別々の経路帯を使用するよう route/via または fromSide/toSide を調整する'
  }));
  problems.push(...cleanAmbiguousCorridorProblems({
    ...
    relationCollection: 'connections',
    profile: arch.meta?.quality_profile,
    routeHint: '無関係な接続が視覚的に合流しないよう route/via または fromSide/toSide を調整する'
  }));
  problems.push(...cleanBorderRunProblems({
    ...
    relationCollection: 'connections',
    profile: arch.meta?.quality_profile,
    routeHint: '接続が境界線に沿わず境界を垂直に横切るよう route/via または fromSide/toSide を調整する'
  }));
  problems.push(...cleanRouteRhythmProblems({
    ...
    relationCollection: 'connections',
    profile: arch.meta?.quality_profile,
    routeHint: '各曲がり角に読み取れる余地ができるよう route/via 点をより広い経路帯へ移動するか、コンポーネントを移動する'
  }));
  ...
  for (const rect of labelRects) {
    for (const c of components.values()) {
      if (rectsOverlap(rect, c, -2)) {
        problems.push(`ラベル "${rect.label}" がコンポーネント "${c.id}" と重なっています。labelDx/labelDy/labelSegment を調整するか labelAt を設定してください。\n${suggestLabelObstacleFix(rect, rect.lx, rect.ly, c)}`);
      }
    }
    if (enforcesBoundaryTitleComposition) {
      for (const boundary of boundaries) {
        ...
        problems.push(
          `境界ラベル "${boundary.label}" が接続ラベル "${rect.label}" と重なっています。内包するコンポーネントの位置を調整して境界タイトル領域を移動するか、labelAt/labelDx/labelDy/labelSegment で接続ラベルを移動してください。`,
        );
      }
    }
  }
  ...
  if (problems.length) {
    throwDiagnosticProblems('アーキテクチャのレイアウト検証に失敗しました', problems, {
